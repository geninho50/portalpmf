...
class gdb
{
  ...
  function conexao(
    $banco    = 'ilhadocampeche-db',
    $servidor = '192.168.12.2:3315',
    $usuario  = 'root',
    $senha    = 'Change1.'
  ) {
  ...
  function open(
    ...
    if (!$this->conexao_lg) {
      ...
      $this->conexao('ilhadocampeche-db', '192.168.12.2:3315', 'root', 'Change1.');
    }
...
// FUNÇÃO DE CADASTRAR PESSOAS SISTEMA ILHA DO CAMPECHE

function cadastrarPessoa( $nome,$cpf,$data,$email,$telefone,$cep,$municipio,$bairro,$endereco,$numero,$complemento,$senha )
{
  $sqlAdmin = "INSERT INTO pessoa( pesnome,pescpf,pesnascimento,pesemail,pestelefone,peslogradouro,pesnumero,pescomplemento,pesbairro,Pesmunicipio,pescep,Pessenha) 
               VALUES('$nome' ,'$cpf', '$data', '$email', '$telefone', '$endereco','$numero', '$complemento','$bairro','$municipio' , '$cep', '$senha')";
  if( $this->open($sqlAdmin) ) return 1;
  else return 0;
}
                                   ...
                                   $id_evento = '' )
    {
      ...
      if( $temCPF>0 ){        
        ...
        return 4;
            ...
            $sqlCheckEndereco = "SELECT id_endereco 
                                   FROM endereco 
                                  WHERE cep = '$cep' ";
            ...
            $sqlPessoa = "INSERT INTO pessoa ( cpf,
                                               nome,
                                               identidade,
                                               nascimento,
                                               email,
    ...
    function incluirEvento($nome_evento,$descricao,$ministrante,$carga_horaria,$local,$vagas,$data,$hora, $status_evento = '1', $observacao)
    {
      ...
      if( $this->open ($sqlEvento,1) ){
          $this->open("SELECT MAX(id_evento) AS ID FROM evento");
          $id_evento = $this->gs["ID"][0];
          $this->open("INSERT INTO statusEvento (id_evento, status_evento)  VALUES ('$id_evento','1') ");
          return 1;
      }
    }
    ...
    {
      $sqlAdmin = "INSERT INTO usuarioAdmin (nome,cpf,email_admin,telefone,celular,cargo,setor,senha) 
                   VALUES                   ('$nome','$cpf','$email_admin','$telefone','$celular','$cargo','$setor','$senha')";
      $this->open($sqlAdmin,1);
