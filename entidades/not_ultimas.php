...
<div class="centro">
    ...
    <br><ul class="listagem">
		...
		$Tcaminho = "?pagina=notultimas&menu=4";
		...
		$numSql = "SELECT COUNT(*) 
			   ...
			   INNER JOIN entidades 
			           ON noticias.noti_entidade_id = entidades.entidade_id 
					WHERE noticias.noti_status = 't' 
					  AND noticias.noti_data <= '$dataAtual' 
					  AND noticias.noti_entidade_id = $noti_id $complemento";
		...
		while ($Tnoticias = pg_fetch_object($TreturnSqlNot)){ // povoa uma matriz com os dados a serem impressos
			...
			$matrizNoticia[$j]['noti_titulo'] = stripcslashes($Tnoticias->noti_titulo);
			$matrizNoticia[$j]['noti_manchete'] = stripcslashes($Tnoticias->noti_manchete);	
		...
		$sql_img = "SELECT 
						imagens.img_link_v_pequena, img_link_v_alta
					FROM 
						noticias_imagens 
					INNER JOIN 
						imagens 
