...
class banco{
	...
	//*Retorno: True se houve sucesso, False se deu erro.
	function conecta(){
	...
	function situcaoSql($lpedido1){
		...
		if( $tem === false ){
			// verifica se tem comando clound
	...
	*/
	function upload($dir,$arquivo,$extensao,$tamanho){
		...
		if($arquivo["size"] > $tamanho) {
			...
			$retorno[2] = iso8859_1_to_utf8($msg); //indice 2 armazena a mensagem
		}else{
	...
	/* Verificação de entrada autorizada */
	function verificarEntrada($entrada){

		$sql = "SELECT *
		          FROM public.entradasexternas 
				 WHERE upper(nomeentrada) = upper('$entrada')";
		...
		// die();
		if( $total > 0 ){
            $sql = "update entradasexternas 
			           set acessosentrada = acessosentrada + 1 
					 where upper(nomeentrada) = upper('$entrada') ";
			$this->conecta();
			$retorno   = pg_query($this->conexao,$sql);
			header("Location: /erro-acesso.html");
			exit();								 
		}else{
			return true;
		}

        // print "Entrada de URL : ".$entrada;
		// die();
