...
if(!isset($_POST['btConsulta_x'])){
	$sqlHistorico = "SELECT * FROM mailing_historico ORDER BY mailing_historico_data DESC LIMIT 20";
}else{
	$TnotiInicio  = inverteDate($_POST['Fdata1']);
	$TnotiFinal   = inverteDate($_POST['Fdata2']);
	$sqlHistorico = "SELECT * FROM mailing_historico WHERE mailing_historico_data >= '$TnotiInicio' AND mailing_historico_data <= '$TnotiFinal' ORDER BY mailing_historico_data DESC ";				
}
$TreturnHist = $drive->pedido($sqlHistorico);

//---------------------------------------------------------------------------
//busca onome de todos os usuários para imprimir como responsável do Mailing
//---------------------------------------------------------------------------
$sqlUsers 	 = "SELECT * FROM uni_usuarios ORDER BY user_id";
$TreturnUser = $drive->pedido($sqlUsers);
while($Tuser = pg_fetch_object($TreturnUser)){
	$Tusuario[$Tuser->user_id] = $Tuser->user_nome;
}
...
<div class="centro">
    ...
    <div id="titulo_pagina">hist&oacute;rico geral</div>
				...
				while($Tmailings = pg_fetch_object($TreturnHist)){
					...
					<tr>
						<td class=\"container_item_result\">  
							<a href=\"inicio.php?pagina=mailhistpagina&menu=".$_GET['menu']."&mid=".$Tmailings->mailing_historico_id."\">
								<strong>".inverteDateBd($Tmailings->mailing_historico_data)." - ".$Tusuario[$Tmailings->mailing_historico_user_id]."</strong>
							</a> 
						</td>
					</tr>";                
				$i++;
