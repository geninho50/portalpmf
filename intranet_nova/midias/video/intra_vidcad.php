...
<div class="centro">
	...
	<div id="titulo_pagina">editar vídeos</div>
	<div id="margem_direita"><br>
		    
		<div class="conteudo_abas">  
       		<form method="post" action="inicio.php?pagina=videcad&menu=<?=$_GET['menu']?>" onsubmit="return habilitaData()">  
			<div id="conteudo_localizar" style="display:inline">
				Utilize as opções abaixo para acesar vídeos do repositório. Faça a pesquisa por palavra-chave (tag), data de postagem ou consulte a listagem completa de vídeos.<br><br>
                ...
                <div class="container_6">
					...
					<input type="text" id="Ftags" name="Ftags" class="ui-widget-content ui-corner-all" value="<?=$Ttags?>"/>De espaço após cada palavra, mesmo que seja apenas uma.<br /><br />               
                	</div>
		...
		if(isset($_POST['btLoc_x']) or (isset($_GET['pg']))){	
			...
			//---------------------------------------
			// busca completa de vídeos da entidade
			//---------------------------------------
			if(($_POST['Ftipo_busca'] == "completa") or ($_GET['tp'] == "cp")){				
				...
				$vidSql   = "SELECT * FROM midia WHERE midia_entidade_id = $TidEntidade AND midia_tipo = 0 ORDER BY midia_data DESC LIMIT 10 OFFSET $inicio";
				$Tcaminho = "?pagina=videcad&menu=".$_GET['menu']."&tp=cp";
			}
			...
			if(($_POST['Ftipo_busca'] == "personalizada") or ($_GET['tp'] == "ps")){		
				...
				if(($_POST['Ftags'] != "") && ($_POST['Fdata2'] == "") or ($_GET['Gtags'] != "")  && ($_GET['dtf'] == "")){
					...
					$Tcaminho	 = "?pagina=videcad&menu=".$_GET['menu']."&tp=ps&Gtags=".$Ttags."";
				}else
					//--------------------------------------------------
				...
				if(($_POST['Fdata2'] != "") && ($_POST['Ftags'] == "") or ($_GET['dtf'] != "")  && ($_GET['Gtags'] == "")){					
					...
					$Tcaminho 	 = "?pagina=videcad&menu=".$_GET['menu']."&tp=ps&dti=".$Tdti."&dtf=".$Tdtf."";
				}
				else 
				//--------------------------------------------------
				...
				if((($_POST['Ftags'] != "") or ($_GET['Gtags'] != "")) && (($_POST['Fdata2'] != "") or ($_GET['dtf'] != "")))
				{
					...
					$Tcaminho		= "?pagina=videcad&menu=".$_GET['menu']."&tp=ps&Gtags=".$Ttags."&dti=".$Tdti."&dtf=".$Tdtf.""; 
				}								
			}
		...
		<div>
			<div class="container_2">  
					...
					$i = 0;
					//---------------------------------------
					// imprime lista de vídeos encontrados
					//---------------------------------------
				...
				<div>
				<span align=\"absmiddle\"><a href=\"?pagina=videdit&menu=".$_GET['menu']."&id=".$Tvid->midia_id."\">
				<img src=\"../layout/imagens/atualiza_btn_editar.png\" border=\"0\"  align=\"absmiddle\" /></a></span>
				<span align=\"absmiddle\"><input type=\"image\" src=\"../layout/imagens/atualiza_btn_excluir.png\" name=\"btExcluir\" value=\"btExcluir\" id=\"btExcluir\" align=\"absmiddle\" onclick=\"return verificaExclusao()\" /></span>
...
if(isset($_POST['btExcluir_x'])){
	...
	if($TverMid != false){
		$Tvalida = 1;
		echo"<script>alert(\"Não foi possível EXCLUIR o Vídeo!\\n\\nEste Vídeo está sendo utilizado em uma Página.\");</script>";
	}
