...
class gdb
{
  ...
  function conexao(
    ...
    $sql = "host=$servidor
		...
		//if( $this->conexao_lg == pg_connect( $sql ) ){    
        $resultado        = pg_connect( $sql );
        $this->conexao_lg = $resultado;

        if ( !$resultado ){      
             print "Nao Acessou o banco !! erro : ". pg_last_error();
             pg_close( $this->conexao_lg );             
        }
    //}
  ...
  function open(
    ...
    if ($utf8) {     
      $this->lg_resl = pg_query($this->conexao_lg, "SET NAMES 'utf8'");
    }
    $this->lg_resl = pg_query($this->conexao_lg, $ds_sql);

    /*
      $numRows = pg_num_rows($this->lg_resl);  
      print '<pre> Total de Registro :'.$numRows.'<br>';
      print $ds_sql;
      print '</pre>';
    */

    $select = stripos($ds_sql, "select");
  
    if ( stripos($ds_sql,"insert") !== false ||
         stripos($ds_sql,"delete") !== false ||
         stripos($ds_sql,"update") !== false )
    { 
      $select = 0;
    }
    ...
    if ($this->lg_resl) { // Se o resultado do query for verdadeiro, pode criar o vetor com a tabela
      ...
      if ($select !== false ) {
        ...
        $this->campos  = pg_num_fields($this->lg_resl); // total de campos da tabela
       
        $this->linhas  = pg_num_rows($this->lg_resl);                
        
        if ( $nu_linh > 1 && $this->linhas > $nu_linh ){
           $this->linhas = $nu_linh;
        }   
        ...
        if ($this->linhas > 0 && $this->campos > 0) {
          for (
            ...
            $registro = pg_fetch_row($this->lg_resl);
            //$registro = pg_fetch_assoc($this->lg_resl);            
            ...
            for (
              ...
              // $nameField = pg_fetch_field_direct($this->lg_resl, $xcampos);
              $nameField = pg_field_name($this->lg_resl, $xcampos);              
              $coluna = strtoupper($nameField);              
              $this->gs[$coluna][$xlinhas] = $registro[$xcampos];
  ...
  function formato($vr_varv, $vr_form)
  {
    ...
    function print_erro()
    {
      echo 'erro n. ' . pg_last_error() . ': ' . pg_last_error() . '<BR>';
      print 'Comando SQL :';
      ...
      if ($this->linhas > 0) {
        ...
        for ($xlin = 0; $xlin < $this->linhas; $xlin++) {
          for ($xcol = 0; $xcol < $this->campos; $xcol++) {
            $xcoluna = strtoupper(pg_field_name($this->lg_resl, $xcol));
      ...
      if ($gdb->linhas > 0) {
        ...
        for ($xlin = 0; $xlin < $gdb->linhas; $xlin++) {
          for ($xcol = 0; $xcol < $gdb->campos; $xcol++) {
            $xcoluna = strtoupper(pg_field_name($gdb->lg_resl, $xcol));
    ...
    function enviarArquivo($doc_rg,$doc_cpf,$doc_residencia)
    {
      ...
      $sqlIdPessoa = "SELECT  d.id_pessoa, p.id_pessoa FROM minhocaCabeca.documentoPessoa d LEFT JOIN minhocaCabeca.pessoa p  WHERE d.id_pessoa = p.id_pessoa";
    ...
    {
      $sqlIdPessoa = "SELECT  p.id_pessoa, d.id_pessoa FROM minhocaCabeca.documentoPessoa d LEFT JOIN minhocaCabeca.pessoa p  WHERE d.id_pessoa = p.id_pessoa";
      $idPessoa = $this->open($sqlIdPessoa);
    ...
    function incluirEvento($nome_evento,$descricao,$ministrante,$carga_horaria,$local,$vagas,$data,$hora)
    {
      $data_e = $data;
      $DFm = explode("/",$data_e);
      $timestamp = $DFm[2].'-'.$DFm[1].'-'.$DFm[0];

      $sqlEvento = "INSERT INTO minhocaCabeca.evento (nome_evento,descricao,ministrante,carga_horaria,local,vagas,data,hora) 
                    VALUES ('$nome_evento','$descricao','$ministrante','$carga_horaria','$local','$vagas','$timestamp','$hora')";

      $this->open($sqlEvento);

      $this->open("SELECT MAX(id_evento) AS ID FROM minhocaCabeca.evento");

      $id_evento = $this->gs["ID"][0];

      $this->open("INSERT INTO minhocaCabeca.statusEvento (id_evento, status_evento) 
                   VALUES ('$id_evento','1')");

      return 1;
    }

    function editarEvento($id_evento, $nome_evento,$descricao,$ministrante,$carga_horaria,$local,$vagas,$data,$hora)
    {
      $data_e = $data;
      $DFm = explode("/",$data_e);
      $timestamp = $DFm[2].'-'.$DFm[1].'-'.$DFm[0];

      $sqlEvento = "UPDATE minhocaCabeca.evento 
    ...
    {
      $sqlEvento = "UPDATE minhocaCabeca.statusEvento SET status_evento = '0' WHERE id_evento = '$id_evento'";
      $this->open($sqlEvento);
    ...
    {
      $sqlDelEvento = "DELETE FROM minhocaCabeca.evento WHERE id_evento = '$id_evento'";
    ...
    function inscricaoEvento($id_pessoa,$id_evento)
    {
      ...
      $selectAtivarUsuario =  "select u.id_usuario as id from minhocaCabeca.pessoa p, minhocaCabeca.usuario u where u.id_pessoa = p.id_pessoa and  p.id_pessoa = '$id_pessoa' "; 
      $this->open( $selectAtivarUsuario );
      $id = $this->gs["ID"][0];

      $this->open("UPDATE minhocaCabeca.statusUsuario SET status_usuario = '1' WHERE id_usuario = '$id' ");
    ...
    function inscricaoEventoAdmin($id_pessoa,$id_evento)
    {
      ...
      $sqlcheckUsuario = "SELECT su.status_usuario FROM minhocaCabeca.usuario u
                          LEFT JOIN minhocaCabeca.pessoa p ON u.id_pessoa = p.id_pessoa
                          LEFT JOIN minhocaCabeca.statusUsuario su ON su.id_usuario = u.id_pessoa
    ...
    function incluirTroca($id_pessoa,$data_troca)
    {
      $data = $data_troca;

      $sqlTroca = "INSERT INTO minhocario (id_pessoa,data_troca,quantidade_troca) VALUES ('$id_pessoa','$data','$quantidade_troca')";
      $this->open($sqlTroca);
    ...
    {
      $this->open("SELECT COUNT(id_pessoa) AS INSCRITOS FROM minhocaCabeca.eventoInscricao WHERE status_inscricao = '1' AND id_evento = '$id_evento'");
      $inscritos = $this->gs["INSCRITOS"][0];

      $this->open("SELECT e.vagas from minhocaCabeca.evento e WHERE id_evento = '$id_evento'");
    ...
    {
      $sqlCheckDocumento = "SELECT count(d.id_pessoa) AS C FROM minhocaCabeca.documentoPessoa d WHERE d.id_pessoa = '$id_pessoa'";
