<!DOCTYPE html>
<html lang="pt-BR">
<head>
    <meta charset="utf-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <title>PMGIRS</title>
    <link type="image/x-icon" rel="shortcut icon" href="img/brasao.gif">
    <link href="css/bootstrap.css" rel="stylesheet">
    <link href="css/estilos.css" rel="stylesheet">
    <script src="js/jquery-2.1.4.min.js" type="text/javascript"></script>
    <link href='https://fonts.googleapis.com/css?family=Cabin+Condensed:700' rel='stylesheet' type='text/css'>
</head>

<body>
    <div class="jumbotron">
        <div>
            <img class ='logo' src="img/logo.png" />
        </div>
    </div>       

    <div class="container">
        <h2 class="text-center">Muito obrigado por participar do Plano Municipal de Gestão Integrada de Resíduos Sólidos - PMGIRS</h2>
        <h2></h2>
     <!--   <img class="foto" src="img/marina1.jpg">
        <img class="foto" src="img/marina2.jpg">-->

 		
 		<div class="row">
              <div class="col-md-6 pull-left">
         <a href="index.php" type="button" name="submit" id="voltar" class="btn btn-primary botao "><b>Voltar</b></a>
            </div>
        </div>

    </div>
</body>
</html>