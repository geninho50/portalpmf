<?php
  ...
  $gdb->open("update caixaRespostaMNC set lido='s' where codigoUsuario = '$codigoUsuario' ");
  ...
  $select = "   select codigoMensagem as codigo, 
					   ...
					   case when ( select count(*) 
							...
							else '<b>Aguardando</b>' end as situacao,
                        mensagem as duvida							
						...
						<? if( $gdb->linhas>0 ){ 
							foreach($gdb->gs['CODIGO'] as $key=>$value){?>
								<tr onclick="mudancaDiv('div<?php print $value;?>');" style="cursor:pointer;">
								   ...
								   <td align="left"><? print $gdb->gs['DUVIDA'][$key]; ?></td>
								 ...
								 <?php									
									...
									if( $gdb3->linhas>0 ){
										...
										$gdb3->titulo_campo = "codigo,Usu&oacute;rio,Data,Duvida,Resposta";
										$gdb3->formato_campo = ",,,,";
									...
									<tr>
										<td align="left" colspan="6">
											<div id="div<?php print $value;?>" style="display:block;">
												<?php 
												  if( $gdb3->linhas>0 ){ $gdb3->print_tabela("Dados do Retorno", 1, 1, ""); }?>
											</div>
										</td>
									</tr>
