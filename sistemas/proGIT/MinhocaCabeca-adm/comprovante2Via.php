...
PDFTermo( $pdf, $db, $login, $senha );

$pdf->Output('termo2Via.pdf', 'I');

function PDFTermo( $pdf, $db, $login, $senha ) {
	 ...
	 $texto ='Eu '.iconv('utf-8','iso-8859-1', $db->gs['NOME'][0] ).', inscrito com o c�digo MNC'.
	              $db->gs['CODIGOPESSOA'][0].' portador do CPF '.
	...
	[X] Participar da oficina de capacitação no dia '.$db->formato($db->gs['DATA'][0],'DD/MM/YYYY').' às '.substr($db->gs['HORA'][0],0,5).'hs ( Turma '.$db->gs['CODIGOTURMA'][0].' ) .
