...
if($_POST["login"] == "pro" && $_POST["senha"] == "*#&MsdI42.") {
?>
...
<head>
  ...
  $gdb->open("SELECT l.documento ,l.nome, c.dia, c.horario 
              FROM inscritos i 
              JOIN login l ON l.documento = i.cpf 
              JOIN cursos c ON c.idcursos = i.idcursos 
  ...
  <div class="mini-header">
    ...
    <div class="flex-container hero-wrapper" id="solicite" >
      <div id="busca-home" class="search-bar search-bar--home column6-lg column8-sm column8-xs">
            <h1 class="hidden-sm hidden-xs">Inscritos</h1>
            <br>
        <table class="table">
          <thead>
            <tr>
          ...
          <tbody>
            <?php foreach( $gdb->gs['DOCUMENTO'] as $key=>$value ) { ?>  
              <tr>
                <td><?=$gdb->gs['DOCUMENTO'][$key]?></td>
                <td><?=$gdb->gs['NOME'][$key]?></td>
                <td><?=$gdb->gs['DIA'][$key]?></td>
                <td><?=$gdb->gs['HORARIO'][$key]?></td>
              </tr>            
            <?php } ?>
          </tbody>
        </table>
