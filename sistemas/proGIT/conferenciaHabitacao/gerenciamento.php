...
if ($user == "admin" && $password == "#p6&e=(c@x%") {
  ...
  $gdb->open("SELECT 
              nome, cpf, email, regiao, cep, logradouro, numero, complemento, bairro, problema, sugestao, justificativa 
            FROM 
              consultaSaneamento");
  ?>
  <!DOCTYPE HTML>
  ...
  <body>
    <div class="page-wrap">
      ...
      <section>
        <div class="inner">
          <header>
            <h2 align="center">Consulta Pública</h2><br />
          </header>
          <div class="column">
            <table>
              <thead>
              ...
              <tbody>
                <?php
                  ...
                  <tr style="font-size: 12px">
                    ...
                    <td><?= $gdb->gs["CEP"][$key] ?></td>
                    <td><?= $gdb->gs["LOGRADOURO"][$key] ?></td>
                    <td><?= $gdb->gs["NUMERO"][$key] ?></td>
                    <td><?= $gdb->gs["COMPLEMENTO"][$key] ?></td>
                    <td><?= $gdb->gs["BAIRRO"][$key] ?></td>
                    <td><?= $gdb->gs["PROBLEMA"][$key] ?></td>
                    <td><?= $gdb->gs["SUGESTAO"][$key] ?></td>
                    <td><?= $gdb->gs["JUSTIFICATIVA"][$key] ?></td>
                  </tr>
                <?php
                  }
                  ?>
