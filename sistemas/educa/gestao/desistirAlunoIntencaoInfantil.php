...
if (isset($_GET['id_aluno'])) {
    if (isset($_GET['motivo'])) {
        if (isset($_GET['escola'])) {
            if (isset($_GET['ano'])) {
                if (isset($_GET['fase'])) {
                    include 'fnc/buscaEscolhaEscola.php';
                    $escolha = buscaEscolhaEscola($_GET['id_aluno'], $_GET['escola']);

                    include 'fnc/removerAlunoIntencao.php';

                    if($escolha[0][0] == 1){
                        $resultado = removerAlunoIntencaoTodas($_GET['id_aluno']);
                    } else {
                        $resultado = removerAlunoIntencao($_GET['id_aluno'], $_GET['escola'], 2, $_GET['fase'], $_GET['ano'], 1);
                    }
                    if ($resultado) {
...
$sql = sprintf("INSERT INTO `matricula`.`auditoria_intencao_infantil_remover`
    (`id_aluno`,
        `id_motivo`,
        `dt_alteracao`,
        `id_usuario`,
...
header('Location: listaDeIntencaoInfantil.php?sucesso=true&escola=' . $_GET['escola'] . '&ano=' . $_GET['ano'] . '&fase=' . $_GET['fase']);
} else {
    header('Location: listaDeIntencaoInfantil.php?sucesso=false&escola=' . $_GET['escola'] . '&ano=' . $_GET['ano'] . '&fase=' . $_GET['fase']);
}
}
}
}
