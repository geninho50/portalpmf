    ...
    if (isset($_GET['ano'])) {
        if (isset($_GET['fase'])) {
            if (($_GET['escola'] == 0) && ($_GET['ano'] == 0) && ($_GET['fase'] == 0)) {
                $matriculados = buscaMatriculados(1);
            } else {
                if (($_GET['escola'] == 0) && ($_GET['ano'] != 0) && ($_GET['fase'] != 0)) {
                    $matriculados = buscaMatriculadosAF($_GET['ano'], $_GET['fase'], 1);
                } else {
                    if (($_GET['escola'] != 0) && ($_GET['ano'] == 0) && ($_GET['fase'] != 0)) {
                        $matriculados = buscaMatriculadosIF($_GET['escola'], $_GET['fase'], 1);
                    } else {
                        if (($_GET['escola'] != 0) && ($_GET['ano'] != 0) && ($_GET['fase'] == 0)) {
                            $matriculados = buscaMatriculadosIA($_GET['escola'], $_GET['ano'], 0,1);
                        } else {
                            if (($_GET['escola'] != 0) && ($_GET['ano'] == 0) && ($_GET['fase'] == 0)) {
                                $matriculados = buscaMatriculadosI($_GET['escola'], 1);
                            } else {
                                if (($_GET['escola'] == 0) && ($_GET['ano'] != 0) && ($_GET['fase'] == 0)) {
                                    $matriculados = buscaMatriculadosA($_GET['ano'], 1);
                                } else {
                                    if (($_GET['escola'] == 0) && ($_GET['ano'] == 0) && ($_GET['fase'] != 0)) {
                                        $matriculados = buscaMatriculadosF($_GET['fase'], 1);
                                    } else {
                                        if (($_GET['escola'] != 0) && ($_GET['ano'] != 0) && ($_GET['fase'] != 0)) {
                                            $matriculados = buscaMatriculadosIAF($_GET['escola'], $_GET['ano'], $_GET['fase'], 1);
                                        }
                                    }
                                }
                            }
...
include 'fnc/verificaAtendidoFundamental.php';
...
<head>
    <meta charset="utf-8">
    <title>SGE &middot; Consulta de Alunos</title>
        ...
        <div class="container">
            ...
            <div class='conteudo' style='min-height: 500px;'>
                <div class='row-fluid' style='text-align: center; padding-top: 20px;'>
                    <h2 style='font-size:30px;'>Consulta de Alunos</h1>
                    </div>
                    <hr>
                    <div class="row-fluid">
                        <div class="span12" style='padding: 20px; padding-top: 0px;'>
                            ...
                            <div id='matriculados' style='display: inherit;'>
                                ...
                                <table class='table table-striped table-bordered' id='tabela'>
                                    <thead>
                                        <tr>
                                            <td style='width: 50px; font-size:10px;'># Matrícula</td>
                                            <td style='width: 50px; font-size:10px;'>Nome</td>
                                            <td style='width: 50px; font-size:10px;'>Nome</td>
                                            <td style='width: 50px; font-size:10px;'>Dt. Nasc.</td>
                                            <td style='width: 50px; font-size:10px;'>Fase</td>
                                            <td style='width: 50px; font-size:10px;'>Tipo</td>
                                            <td style='width: 50px; font-size:10px;'>Efetivado</td>
                                        ...
                                        if (isset($matriculados)) {
                                            if ($matriculados != false) {
                                                foreach ($matriculados as $key => $value) {
                                                    echo '<tr>';
                                                    echo '<td style=" font-size:12px; width: 50px;">';
                                                    echo $value[0];
                                                    echo '</td>';
                                                    echo '<td style=" font-size:10px; width: 180px;">';
                                                    echo ($value[1]);
                                                    echo '</td>';
                                                    echo '<td style=" font-size:10px; width: 120px;">';
													$escola = buscaEscola($value[8]);
                                                    echo $escola[1][1];
                                                    echo '</td>';
                                                    echo '<td style=" font-size:10px; width: 60px;">';
                                                    echo ($value[2]);
                                                    echo '</td>';
                                                    echo '<td style=" font-size:10px; width: 60px;">';
                                                    ...
                                                    echo '<a href="relatorios/emitirFormCadastroFundamental.php?idAluno='.$value[4].'"><i class="iconic-document-alt-stroke" style="color: green"></i></a>';
                                                    echo ' ';
                                                    if($_SESSION['usuario']['perfil'] != 11){
                                                        if($_SESSION['usuario']['perfil'] != 14){
                                                            if ($value[5] != 1) {
                                                                if(($value[8] == $_SESSION['usuario']['id_escola']) || ($_SESSION['usuario']['id_escola'] == null)){
...
//                                                    echo '&nbsp;';
                                                            ...
                                                            if($_SESSION['usuario']['permissoes'][12][1] == 1){
                                                                if(verificaAtendidoFundamental($value[4], $value[8], $value[7], 2014, 1, 1)){
                                                                    echo '&nbsp;';
                                                                    echo '<a title="Reverter" href="#modalReverter" role="button" data-toggle="modal"
                                                                    onclick="passarInfoR(' . $value[0] . ', ' . $aspa . strtoupper(($value[1])) . $aspa . ',' . ($value[4]) . ');"
                                                                    ><i class="iconic-x" style="color: #cccc00;"></i></a>';
                                                                }
                                                                echo '&nbsp;';
                                                    ...
                                                    echo '&nbsp;'; 
                                                    echo '<a title="Ocorrências" href="ocorrencias.php?idAluno=' . $value[4] . '"><i class="iconic-book" style="color: orange"></i></a>';
                                                    echo '</td>';
                                                    echo '</tr>';
                                                }
                                            }
                ...
                <div id="modalReverter" class="modal hide fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
                    <div class="modal-header">
                        <button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
                        <h3 id="labelModalEfetivar"><strong># Matrícula:</strong> <font id='numeroMatriculaR'></font> &middot; <font id='nomeAlunoR'></font></h3>
                    </div>
                    <div class="modal-body">
                        <p>Tem certeza que você deseja reverter este atendimento?</p>
                    </div>
                    <div class="modal-footer">
                        <form action='reverterAtendimentoFundamental.php' method='post'>
                            <input type='hidden' name='id_aluno' id='inputId'>
                            <a class="btn" data-dismiss="modal" aria-hidden="true">Não</a>
                            <button class="btn btn-primary">Sim</button>
                        </form>
                    </div>
                ...
                <div id="modalDesistir" class="modal hide fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
                    ...
                    <div class="modal-body">
                        <p>Tem certeza que você deseja desistir o aluno desta vaga?</p>
                    </div>
                    <div class="modal-footer">
        ...
        <script type="text/javascript">
            ...
            function passarInfoR(matricula, nome, id_aluno) {
                $('#numeroMatriculaR').html(matricula);
                $('#nomeAlunoR').html(nome);
                $('#inputId').val(id_aluno);
            }
