...
if (count($_POST) > 0) {
    ...
    if (!isset($erro)) {
        ...
        if($resultado == true){
            $sucesso = true;
            include_once 'fnc/insereAuditoriaEditarLocalizacaoInfantil.php';
            insereAuditoriaEditarLocalizacaoInfantil($_SESSION['localizacao'], $_GET['idAluno'], $_SESSION['usuario']['id']);
        } else {
            $sucesso = false;
        }
    }
