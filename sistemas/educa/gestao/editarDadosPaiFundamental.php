...
$pai = (buscaDadosPai($_GET['idAluno']));
...
if (isset($_SESSION['identificacao']['nome_pai'])) {
    $dadosPessoais['nome'] = $_SESSION['identificacao']['nome_pai'];
}


if ($pai != false) {
    $dadosPessoais['preenchido'] = true;
    $dadosPessoais['nome'] = $pai[9];
    $data = explode('-', $pai[10]);
    $dadosPessoais['data_nascimento'] = $data[2] . '/' . $data[1] . '/' . $data[0];
    $dadosPessoais['cpf'] = substr($pai[11], 0, 3) . '.' . substr($pai[11], 3, 3) . '.' . substr($pai[11], 6, 3) . '-' . substr($pai[11], 9, 2);
    $dadosPessoais['sexo'] = strtolower($pai[8]);
    $dadosPessoais['estado_civil'] = ($pai[4]);
    $dadosPessoais['etnia'] = ($pai[1]);
    $dadosPessoais['escolaridade'] = ($pai[7]);
    $dadosPessoais['profissao'] = ($pai[3]);
    $dadosPessoais['religiao'] = ($pai[2]);
    $dadosPessoais['nacionalidade'] = ($pai[5]);
...
//cep
    ...
    if (!isset($erro)) {
        ...
        if(isset($pai[0])){
            if($pai[0] != ''){
                $resultado = (atualizarResponsavelEditar($dadosPessoais, $pai[0], 'pai', $_GET['idAluno']));
            }
        } else {
            $resultado = (inserirResponsavelEditar($dadosPessoais, null, 'pai', $_GET['idAluno']));
        }
        $sucesso = $resultado;
        if($sucesso == true){
            include_once 'fnc/insereAuditoriaEditarDadosPaiFundamental.php';
            insereAuditoriaEditarDadosPaiFundamental($dadosPessoais, $_GET['idAluno'], $_SESSION['usuario']['id']);
        }
    }
}
...
<head>
    <meta charset="utf-8">
    <title>SGE &middot; Dados do Pai do Aluno</title>
                        ...
                        <div class="span12" style='padding: 20px; padding-top: 0px;'>
                            ...
                            if(isset($sucesso)){
                                if($sucesso){
                                    ?>
                                    <div class="sucesso">
                                        <strong>Sucesso!</strong> Dados do Pai atualizados!
                                    </div>
                                    <?php
                                } else {
                                    ?>
