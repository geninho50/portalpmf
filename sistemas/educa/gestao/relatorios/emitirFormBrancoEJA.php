...
// Put a hyperlink
    ...
    function CertidaoVelha($termo, $folha, $livro, $cartorio, $uf_cart) {
        ...
        if ($uf_cart > 1) {
            if ($uf_cart < 28) {
            	$nomeEstado = buscaNomeEstado($uf_cart);
                $uf_cart = $nomeEstado[2];
            }
        }
...
$nacionalidade = buscaNacionalidade($_SESSION['dados_pessoais']['nacionalidade']);
$pais = $nacionalidade[1][1];
include '../fnc/buscaMunicipio.php';
include '../fnc/buscaEstado.php';
if ($pais == 'Brasil') {
	$municipio = buscaMunicipio($_SESSION['dados_pessoais']['naturalidade']['municipio']);
    $cidade = ($municipio[1][1]);
	$nomeEstado = buscaNomeEstado($_SESSION['dados_pessoais']['naturalidade']['uf']);
    $estado = $nomeEstado[2];
}
...
if (isset($_SESSION['escola']['id'])) {
	$escola = buscaEscola($_SESSION['escola']['id']);
    $unidadeInscricao = ($escola[$_SESSION['escola']['id']][1]);
} else {
    $unidadeInscricao = (buscaEscolaRematricula($_SESSION['id']));
	$escola = buscaEscola($unidadeInscricao[7]);
    $unidadeInscricao = $escola[$unidadeInscricao[7]][1];
}
...
if (isset($_SESSION['mae'])) {
    ...
    if (isset($_SESSION['mae']['data_nascimento'])) {
		...
		$etnia = buscaEtnia($_SESSION['mae']['etnia']);
        $etniaMae = $etnia[1];
		$religiao = buscaReligiao($_SESSION['mae']['religiao']);
        $religiaoMae = $religiao[1];
