...
// Page footer
    ...
    // Data
        ...
        foreach($anos as $key => $value)
        {
            ...
            $this->Cell($w[2],6,$total[$value],'LR',0,'C',$fill);
            $this->Ln();
            $fill = !$fill;
        }
...
$header[1] = utf8_decode('Vagas de Reserva Preenchidas');
//$header[2] = utf8_decode('Vagas de Rematricula');
$header[2] = utf8_decode('Total de Vagas de Reserva');
...
foreach ($escolas as $key => $value) {
    $qtVagas = buscaVagasEscolaFundamentalReserva($value[0]);
...
if($alguma == false){
        ...
        $pdf->Cell(190, 6, iconv('utf-8','iso-8859-1', "Nenhuma vaga de reserva no sistema."), 0,0,'C', false);
}
