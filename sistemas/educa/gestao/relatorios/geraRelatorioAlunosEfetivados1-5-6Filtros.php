...
if(!isset($_GET['fase'])){
    header("Location: relatoriosRematricula.php");
}
...
$pdf->Ln(6);
$pdf->Cell(0, 10, utf8_decode('Relatório - Alunos com rematrícula realizada e efetivados'),0,0,'C');
$pdf->Ln(12);
...
$sql = sprintf("select b.id_inscricao, 
    ...
    order by c.ds_nome
    limit 103000", mysql_real_escape_string($_GET['fase']), mysql_real_escape_string($_GET['escola']));
...
$buscaEscola = buscaEscola($_GET['escola']);

$pdf->Cell(0, 10, $buscaEscola[1][1],0,0,'C');
$pdf->Ln(6);
$pdf->Cell(0, 10, utf8_decode($fase),0,0,'C');
$pdf->Ln(12);
if(!isset($datas)){
    $pdf->SetFont('Arial','',16);
    $pdf->Cell(0, 10, utf8_decode('Não há alunos rematriculados e efetivados'),0,0,'C');
} else{
    $data = $datas;
