...
$celularAluno = '';
  ...
  include_once '../fnc/buscaTel.php';
    ...
    if (isset($telefones2[1])) {
        $dados['dados_pessoais']['telefones']['residencial'] = $telefones2[1][3] . $telefones2[1][4];
    }
    if (isset($telefones2[2])) {
        $dados['dados_pessoais']['telefones']['comercial'] = $telefones2[2][3] . $telefones2[2][4];
    }
    if (isset($telefones2[3])) {
        $dados['dados_pessoais']['telefones']['celular'] = $telefones2[3][3] . $telefones2[3][4];
    }
...
// Arial bold 15
$pdf->SetFont('Arial', 'B', 12);
$pdf->Cell(0, 0, iconv('utf-8', 'iso-8859-1', 'Formulário de Cadastro de Aluno '.date('Y').' - Educação Infantil (Intenção)'), 0, 0, 'C');
$pdf->Ln(10);
// Line break
...
if(!isseT($dados['escola']['grupo'])){
    $grupo = 'Nenhum';
    $dados['escola']['grupo'] = -1;
}

if($dados['escola']['grupo'] == 10){
    $grupo = 'Grupo 1';
}
if($dados['escola']['grupo'] == 11){
    $grupo = 'Grupo 2';
}
if($dados['escola']['grupo'] == 12){
    $grupo = 'Grupo 3';
}
if($dados['escola']['grupo'] == 13){
    $grupo = 'Grupo 4';
}
if($dados['escola']['grupo'] == 14){
    $grupo = 'Grupo 5';
}
if($dados['escola']['grupo'] == 15){
    $grupo = 'Grupo 6';
}
$pdf->Ln(6);
