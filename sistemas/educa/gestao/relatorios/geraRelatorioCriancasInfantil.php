...
// Page footer
    ...
    // Data
        ...
        {
            $this->Cell(43);
            $varBuscaBairro = buscaBairro($row[0]);
            $buscaBairro = utf8_decode($varBuscaBairro[$row[0]][1]);
            $this->Cell($w[0],6,$buscaBairro,'LR',0,'C',$fill);
            ...
            $this->Cell($w[1],6,($somaB),'LRT',0,'C',$fill);
            $this->Cell($w[1],6,floor(($somaA/($somaA+$somaB))*100).'%','LRT',0,'C',$fill);
            $this->Ln();
...
$pdf->Ln(12);
$header = array('Bairro', 'Atendidos', utf8_decode('Em intenção'), 'Atendidos (%)');
