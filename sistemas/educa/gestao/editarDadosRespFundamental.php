...
$resp = (buscaDadosResponsavel($_GET['idAluno']));

include_once 'fnc/buscaComQuemMora.php';
$cqm = buscaComQuemMoraParentesco($_GET['idAluno'], 3);
if($cqm == true){
...
$qa = buscaQuemAcompanhaParentesco($_GET['idAluno'], 3);
if($qa == true){
...
if ($resp[5] == 30) {
    ...
    $naturalidade = buscaNaturalidade($resp[0]);
}

if (isset($_SESSION['identificacao']['nome_resp'])) {
    $dadosPessoais['nome'] = $_SESSION['identificacao']['nome_resp'];
}


if ($resp != false) {
    $dadosPessoais['preenchido'] = true;
    $dadosPessoais['nome'] = $resp[9];
    $data = explode('-', $resp[10]);
    $dadosPessoais['data_nascimento'] = $data[2] . '/' . $data[1] . '/' . $data[0];
    $dadosPessoais['cpf'] = substr($resp[11], 0, 3) . '.' . substr($resp[11], 3, 3) . '.' . substr($resp[11], 6, 3) . '-' . substr($resp[11], 9, 2);
    $dadosPessoais['sexo'] = strtolower($resp[8]);
    $dadosPessoais['estado_civil'] = ($resp[4]);
    $dadosPessoais['etnia'] = ($resp[1]);
    $dadosPessoais['escolaridade'] = ($resp[7]);
    $dadosPessoais['profissao'] = ($resp[3]);
    $dadosPessoais['religiao'] = ($resp[2]);
    $dadosPessoais['nacionalidade'] = ($resp[5]);
    ...
    $end = buscaTrabalhoEndereco($resp[0]);
    if ($end != false) {
...
//cep
    ...
    if (!isset($erro)) {
        ...
        if(isset($resp[0])){
            if($resp[0] != ''){
                $resultado = (atualizarResponsavelEditar($dadosPessoais, $resp[0], 'resp', $_GET['idAluno']));
            }
        } else {
            $resultado = (inserirResponsavelEditar($dadosPessoais, null, 'resp', $_GET['idAluno']));            
        }
        $sucesso = $resultado;
        if($sucesso == true){
            include_once 'fnc/insereAuditoriaEditarDadosRespFundamental.php';
            insereAuditoriaEditarDadosRespFundamental($dadosPessoais, $_GET['idAluno'], $_SESSION['usuario']['id']);
        }
    }
}
...
<head>
    <meta charset="utf-8">
    <title>SGE &middot; Dados do Responsável do Aluno</title>
    ...
    <body>
        ...
        <div class="container">
            ...
            <div class='conteudo' style='min-height: 500px;'>
                        ...
                        <div class="span12" style='padding: 20px; padding-top: 0px;'>
                            ...
                            if(isset($sucesso)){
                                if($sucesso){
                                    ?>
                                    <div class="sucesso">
                                        <strong>Sucesso!</strong> Dados do Responsável atualizados!
                                    </div>
                                    <?php
                                } else {
                                    ?>
                ...
                $respses = listaDePaises();
                foreach ($respses as $key => $value) {
                    if (isset($_POST['nacionalidade'])) {
