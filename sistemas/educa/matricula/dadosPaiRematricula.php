...
include 'fnc/buscaDadosPai.php';
$pai = (buscaDadosPai($_SESSION['id']));

if ($pai[5] == 30) {
    include 'fnc/buscaNaturalidade.php';
    $naturalidade = buscaNaturalidade($pai[0]);
}

if (isset($_SESSION['identificacao']['nome_pai'])) {
    $_SESSION['pai']['nome'] = $_SESSION['identificacao']['nome_pai'];
}


if ($pai != false) {
    $_SESSION['pai']['preenchido'] = true;
    $_SESSION['pai']['nome'] = $pai[9];
    $data = explode('-', $pai[10]);
    $_SESSION['pai']['data_nascimento'] = $data[2] . '/' . $data[1] . '/' . $data[0];
    $_SESSION['pai']['cpf'] = substr($pai[11], 0, 3) . '.' . substr($pai[11], 3, 3) . '.' . substr($pai[11], 6, 3) . '-' . substr($pai[11], 9, 2);
    $_SESSION['pai']['sexo'] = strtolower($pai[8]);
    $_SESSION['pai']['estado_civil'] = ($pai[4]);
    $_SESSION['pai']['etnia'] = ($pai[1]);
    $_SESSION['pai']['escolaridade'] = ($pai[7]);
    $_SESSION['pai']['profissao'] = ($pai[3]);
    $_SESSION['pai']['religiao'] = ($pai[2]);
    $_SESSION['pai']['nacionalidade'] = ($pai[5]);
    iF (isset($naturalidade)) {
        $_SESSION['pai']['naturalidade'] = ($naturalidade[1]);
        $_SESSION['pai']['naturalidade_municipio'] = ($naturalidade[0]);
    }

    include 'fnc/buscaTel.php';
    $telefones = buscaTel($pai[0]);
    if (isset($telefones[1])) {
        $_SESSION['pai']['celular'] = $telefones[1][3] . $telefones[1][4];
    }
    if (isset($telefones[2])) {
        $_SESSION['pai']['telefone'] = $telefones[2][3] . $telefones[2][4];
    }
    if (isset($telefones[3])) {
        $_SESSION['pai']['comercial'] = $telefones[3][3] . $telefones[3][4];
    }

    include 'fnc/buscaEmail.php';
    $email = buscaEmail($pai[0]);
    if ($email != false) {
        $_SESSION['pai']['email'] = $email[0];
    }

    include 'fnc/buscaAlunoEndereco.php';
    $end = buscaAlunoEndereco($pai[0]);

    $_SESSION['pai']['cep'] = substr($end[11], 0, 2) . '.' . substr($end[11], 2, 3) . '-' . substr($end[11], 5, 3);
    $_SESSION['pai']['logradouro'] = $end[6];
    $_SESSION['pai']['numero'] = $end[7];
    $_SESSION['pai']['complemento'] = $end[8];
    $_SESSION['pai']['bairro'] = $end[3];
    $_SESSION['pai']['estado'] = $end[5];
    $_SESSION['pai']['municipio'] = $end[4];

    $end = buscaTrabalhoEndereco($pai[0]);
    if ($end != false) {
        $_SESSION['pai']['cep_trabalho'] = substr($end[11], 0, 2) . '.' . substr($end[11], 2, 3) . '-' . substr($end[11], 5, 3);
        $_SESSION['pai']['logradouro_trabalho'] = $end[6];
        $_SESSION['pai']['numero_trabalho'] = $end[7];
        $_SESSION['pai']['complemento_trabalho'] = $end[8];
        $_SESSION['pai']['bairro_trabalho'] = $end[3];
        $_SESSION['pai']['estado_trabalho'] = $end[5];
        $_SESSION['pai']['municipio_trabalho'] = $end[4];
    }

    include 'fnc/buscaTurnos.php';
    $temp = (buscaTurnos($pai[0]));
    $_SESSION['pai']['turnos'] = array();
    if ($temp[0] == 1) {
        array_push($_SESSION['pai']['turnos'], 'mat');
    }
    if ($temp[1] == 1) {
        array_push($_SESSION['pai']['turnos'], 'ves');
    }
    if ($temp[2] == 1) {
        array_push($_SESSION['pai']['turnos'], 'not');
    }
...
//cep
    ...
    if (!isset($erro)) {
        ...
        if ($_SESSION['dados_pessoais']['quem_acompanha'] == 'pai') {
            ...
            $_SESSION['pai']['quem_acompanha'] = false;
        }
        include 'fnc/inserirResponsavel.php';
        include 'fnc/atualizarResponsavel.php';
        if(isset($pai[0])){
            if($pai[0] != ''){
                (atualizarResponsavel($_SESSION['pai'], $pai[0], 'pai', $_SESSION['id']));
            }
        } else {
            if (!isset($_SESSION['preenchido']['pai'])) {
                (inserirResponsavel($_SESSION['pai'], null, 'pai', $_SESSION['id']));
                $_SESSION['preenchido']['pai'] = true;
            } else {
                if ($_SESSION['preenchido']['pai'] != true) {
                    (inserirResponsavel($_SESSION['pai'], $_SESSION['pai']['id'], 'pai', $_SESSION['id']));
                    $_SESSION['preenchido']['pai'] = true;
                }
            }
        }
        ...
        header("Location: dadosResponsavelRematricula.php");
    }
}
...
<a class='btn btn-warning' href='dadosResponsavelRematricula.php' <?php
if ($moraComAluno) {
    echo 'style="display: none;"';
}
?>>Pular</a>
