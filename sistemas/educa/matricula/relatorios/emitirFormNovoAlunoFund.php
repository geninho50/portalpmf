...
// Put a hyperlink
    ...
    function RgAluno($rg, $orgao, $uf, $data) {
        ...
        $this->Ln(6);
        $this->SetFont('Arial', 'B', 11);
        $this->Cell(5, 5, iconv('utf-8', 'iso-8859-1', '#: '), 0, 0, 'C');
        $this->SetFont('Arial', '', 11);
        $this->Cell(48, 5, iconv('utf-8', 'iso-8859-1', $rg), 1, 0, 'C');
        $this->SetFont('Arial', 'B', 11);
    ...
    function DadosMae($nome, $sexo, $dataNasc, $etnia, $estadoCivil, $escolaridade, $religiao, $profissao, $cpf) {
        ...
        $this->Cell(5, 5, iconv('utf-8', 'iso-8859-1', $tempTres), 1, 0, 'C');
        $this->Cell(30, 5, iconv('utf-8', 'iso-8859-1', 'Divorciado:'), 0, 0, 'C');
        $this->Cell(5, 5, iconv('utf-8', 'iso-8859-1', $tempQuatro), 1, 0, 'C');
        $this->Cell(30, 5, iconv('utf-8', 'iso-8859-1', 'União Estável:'), 0, 0, 'C');
    ...
    function DadosEscola($unidadeInscricao) {
        ...
        $this->Cell(48, 5, iconv('utf-8', 'iso-8859-1', 'Dados de Matrícula:'), 0, 0, 'C');
        $this->SetFont('Arial', 'B', 11);
        $this->SetFont('Arial', 'B', 11);
        $this->Ln(6);
        ...
        $ano = buscaAnoRematricula($_SESSION['aluno']['id']);
        if ($ano[0] == 1) {
            $ano = 'Primeiro Ano';
        } else {
            if ($ano[0] == 2) {
...
$possuiIrmaos = $_SESSION['escola']['novaEscola']['possui_irmaos'];
...
$alergia = $_SESSION['saude']['outros']['alergia'];
...
if (isset($_SESSION['escola']['novaEscola']['id_escola'])) {
    ...
    $unidadeInscricao = (buscaEscola($_SESSION['escola']['novaEscola']['id_escola'])[$_SESSION['escola']['novaEscola']['id_escola']][1]);
} else {
    $unidadeInscricao = (buscaEscolaFund($_SESSION['aluno']['id']));
...
if (isset($_SESSION['mae'])) {
    ...
    if (isset($_SESSION['mae']['data_nascimento'])) {
        ...
        if (isset($_SESSION['mae']['telefone'])) {
            if(strlen($_SESSION['mae']['telefone']) > 2){
                $residencialMae = '(' . substr($_SESSION['mae']['telefone'], 0, 2) . ')' . substr($_SESSION['mae']['telefone'], 2);
            }
        } else {
            $residencialMae = '';
        }
        if (isset($_SESSION['mae']['comercial'])) {
            if(strlen($_SESSION['mae']['comercial']) > 2){
                $comercialMae = '(' . substr($_SESSION['mae']['comercial'], 0, 2) . ')' . substr($_SESSION['mae']['comercial'], 2);
            }
        } else {
            $comercialMae = '';
        }
        if (isset($_SESSION['mae']['celular'])) {
            if(strlen($_SESSION['mae']['celular']) > 2){
             $celularMae = '(' . substr($_SESSION['mae']['celular'], 0, 2) . ')' . substr($_SESSION['mae']['celular'], 2);
         }
     } else {
        $celularMae = '';
    }
...
if (isset($_SESSION['pai'])) {
    ...
    if (isset($_SESSION['pai']['data_nascimento'])) {
        ...
        if (isset($_SESSION['pai']['telefone'])) {
            if(strlen($_SESSION['pai']['telefone']) > 2){
                $residencialPai = '(' . substr($_SESSION['pai']['telefone'], 0, 2) . ')' . substr($_SESSION['pai']['telefone'], 2);
            }
        } else {
            $residencialPai = '';
        }
        if (isset($_SESSION['pai']['comercial'])) {
            if(strlen($_SESSION['pai']['comercial']) > 2){
                $comercialPai = '(' . substr($_SESSION['pai']['comercial'], 0, 2) . ')' . substr($_SESSION['pai']['comercial'], 2);
            }
        } else {
            $comercialPai = '';
        }
        if (isset($_SESSION['pai']['celular'])) {
            if(strlen($_SESSION['pai']['celular']) > 2){
             $celularPai = '(' . substr($_SESSION['pai']['celular'], 0, 2) . ')' . substr($_SESSION['pai']['celular'], 2);
         }
     } else {
        $celularPai = '';
    }
...
if (isset($_SESSION['responsavel'])) {
    ...
    if (isset($_SESSION['responsavel']['data_nascimento'])) {
        ...
        if (isset($_SESSION['responsavel']['telefone'])) {
            if(strlen($_SESSION['responsavel']['telefone']) > 2){
                $residencialResp = '(' . substr($_SESSION['responsavel']['telefone'], 0, 2) . ')' . substr($_SESSION['responsavel']['telefone'], 2);
            }
        } else {
            $residencialResp = '';
        }
        if (isset($_SESSION['responsavel']['comercial'])) {
            if(strlen($_SESSION['responsavel']['comercial']) > 2){
                $comercialResp = '(' . substr($_SESSION['responsavel']['comercial'], 0, 2) . ')' . substr($_SESSION['responsavel']['comercial'], 2);
            }
        } else {
            $comercialResp = '';
        }
        if (isset($_SESSION['responsavel']['celular'])) {
            if(strlen($_SESSION['responsavel']['celular']) > 2){
             $celularResp = '(' . substr($_SESSION['responsavel']['celular'], 0, 2) . ')' . substr($_SESSION['responsavel']['celular'], 2);
         }
     } else {
        $celularResp = '';
    }
...
// Arial bold 15
$pdf->SetFont('Arial', 'B', 12);
$pdf->Cell(0, 0, iconv('utf-8', 'iso-8859-1', 'Formulário de Matrícula de Aluno - Educação Fundamental'), 0, 0, 'C');
$pdf->Ln(10);
// Line break
...
if(isset($_SESSION['saude']['outros']['alergia'])){
    $desAlergia = $_SESSION['saude']['outros']['desAlergia'];
} else {
    $desAlergia = '';
}
...
if(isset($_SESSION['saude']['outros']['encaminha'])){
    if($_SESSION['saude']['outros']['encaminha'] == 1){
        $encaminha = 'Sim';
    } else {
        if($_SESSION['saude']['outros']['encaminha'] == 0){
            $encaminha = 'Não' ;
        }
    }
} else {
...
foreach ($_SESSION['renda']['pessoas'] as $key => $value) {
    $data[$key] = [$value[0], buscaSituacaoOcupacional($value[1])[1], utf8_decode($value[4]), $value[3], $value[2], utf8_decode(buscaComprovacao($value[5])[1])];
}
