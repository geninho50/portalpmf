...
// Put a hyperlink
    ...
    function Endereco($cep, $logradouro, $numero, $complemento, $bairro, $cidade, $uf) {
        ...
        $this->SetFont('Arial', '', 11);
        $this->Cell(50, 5, $bairro, 1, 0, 'C');
        $this->Ln(6);
        $this->SetFont('Arial', 'B', 11);
        $this->Cell(21, 5, iconv('utf-8', 'iso-8859-1', 'Cidade: '), 0, 0, 'C');
        $this->SetFont('Arial', '', 11);
        $this->Cell(50, 5, $cidade, 1, 0, 'C');
        $this->SetFont('Arial', 'B', 11);
        $this->Cell(10, 5, iconv('utf-8', 'iso-8859-1', 'UF:'), 0, 0, 'C');
        $this->SetFont('Arial', '', 10);
        $this->Cell(8, 5, iconv('utf-8', 'iso-8859-1', $uf), 1, 0, 'C');
    ...
    function DadosMae($nome, $sexo, $dataNasc, $etnia, $estadoCivil, $escolaridade, $religiao, $profissao, $cpf) {
        ...
        $this->SetFont('Arial', '', 11);
        $this->Cell(40, 5, $religiao, 1, 0, 'C');
        $this->SetFont('Arial', 'B', 11);
        $this->Cell(26, 5, iconv('utf-8', 'iso-8859-1', 'Profissão*: '), 0, 0, 'C');
        $this->SetFont('Arial', '', 11);
        $this->Cell(40, 5, $profissao, 1, 0, 'C');
        $this->SetFont('Arial', 'B', 11);
    ...
    function DadosEscola($unidadeInscricao) {
        ...
        $this->SetFont('Arial', '', 11);
        $this->Cell(100, 5, $unidadeInscricao, 1, 0, 'C');
        $this->Ln(6);
...
$rgAluno = buscaDocumento($_SESSION['id'], 1);
...
if ($pais == 'Brasil') {
    $cidade = utf8_decode(buscaMunicipio($_SESSION['dados_pessoais']['naturalidade']['municipio'])[1][1]);
    $estado = utf8_decode(buscaNomeEstado($_SESSION['dados_pessoais']['naturalidade']['uf'])[2]);
}
...
if (isset($_SESSION['dados_pessoais']['telefones']['residencial'])) {
    $residencialAluno = '(' .
            substr($_SESSION['dados_pessoais']['telefones']['residencial'], 0, 2) . ')' .
            substr($_SESSION['dados_pessoais']['telefones']['residencial'], 2);
}
if (isset($_SESSION['dados_pessoais']['telefones']['comercial'])) {
    $comercialAluno = '(' .
            substr($_SESSION['dados_pessoais']['telefones']['comercial'], 0, 2) . ')' .
            substr($_SESSION['dados_pessoais']['telefones']['comercial'], 2);
}
if (isset($_SESSION['dados_pessoais']['telefones']['celular'])) {
    $celularAluno = '(' .
            substr($_SESSION['dados_pessoais']['telefones']['celular'], 0, 2) . ')' .
            substr($_SESSION['dados_pessoais']['telefones']['celular'], 2);
}
...
$pdf->Ln(6);
$pdf->Endereco($cep, $logradouro, $numeroEnd, $complemento, $bairro, iconv('utf-8', 'iso-8859-1', 'Florianópolis'), "SC");
$pdf->Ln(6);
