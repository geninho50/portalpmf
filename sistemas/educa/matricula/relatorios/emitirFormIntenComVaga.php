...
// Put a hyperlink
    ...
    function NovaEscola($escolaNova, $anoNova, $possuiIrmaos, $motivo) {
        $this->SetFont('Arial', 'BU', 13);
        $this->Cell(70, 5, iconv('utf-8', 'iso-8859-1', 'Unidade Educativa da Intenção:'), 0, 0, 'C');
        $this->Ln(6);
        $this->SetFont('Arial', 'B', 11);
...
$infoAluno = buscaInfoAluno($_SESSION['aluno']['id']);

include '../fnc/buscaDocumento.php';
$rgAluno = buscaDocumento($_SESSION['aluno']['id'], 1);
...
$certAluno = buscaDocumento($_SESSION['aluno']['id'], 2);
...
$certAluno = buscaDocumento($_SESSION['aluno']['id'], 4);
if($certAluno != false){
...
$endAluno = buscaAlunoEndereco($_SESSION['aluno']['id']);
...
if ($dados['localizacao']['bairro']) {
    if ($dados['localizacao']['bairro'] != '') {
        ...
        $bairro = buscaBairro($dados['localizacao']['bairro'])[$id][1];
    }
}
...
$anoNova = buscaAno($_SESSION['escola']['novaEscola']['ano'], 1, $_SESSION['escola']['novaEscola']['id_escola'], 1, 2014)[1];
...
$pdf->Ln(6);
$pdf->NovaEscola($escolaNova, $anoNova);
$pdf->Ln(6);
