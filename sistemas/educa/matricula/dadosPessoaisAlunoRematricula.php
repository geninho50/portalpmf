...
session_name('re');
session_start();
if (!$_SESSION['preenchido']['identificacao']) {
    header("Location: identificacaoRematricula.php");
}
...
$comQuemMora = buscaComQuemMora($_SESSION['id']);
if (isset($comQuemMora)) {
    if ($comQuemMora != false) {
            $_SESSION['dados_pessoais']['com_quem_mora'] = array();
        foreach ($comQuemMora as $key => $value) {
            if ($value[2] == 1) {
                array_push($_SESSION['dados_pessoais']['com_quem_mora'], 'mae');
            }
            if ($value[2] == 2) {
                array_push($_SESSION['dados_pessoais']['com_quem_mora'], 'pai');
            }
            if ($value[2] == 3) {
                array_push($_SESSION['dados_pessoais']['com_quem_mora'], 'outro');
            }
        }
    }
}
...
$quemAcompanha = buscaQuemAcompanha($_SESSION['id']);
if (isset($quemAcompanha)) {
    if ($quemAcompanha != false) {
        foreach ($quemAcompanha as $key => $value) {
            if ($value[2] == 1) {
                $_SESSION['dados_pessoais']['quem_acompanha'] = 'mae';
            } else {
                if ($value[2] == 2) {
                    $_SESSION['dados_pessoais']['quem_acompanha'] = 'pai';
                } else {
                    if ($value[2] == 3) {
                        $_SESSION['dados_pessoais']['quem_acompanha'] = 'outro';
                    }
                }
            }
        }
...
$alterado = false;
if (isset($_POST['sexo'])) {
    if ($_POST['sexo'] != $_SESSION['dados_pessoais']['sexo']) {
        $alterado = true;
    }
}
if (isset($_POST['etnia'])) {
    if ($_POST['etnia'] != $_SESSION['dados_pessoais']['etnia']) {
        $alterado = true;
    }
}
//todo
...
if (count($_POST) > 0) {
    if (!isset($erro)) {
        ...
        $_SESSION['dados_pessoais']['com_quem_mora'] = $_POST['comQuemMora'];
        $_SESSION['dados_pessoais']['quem_acompanha'] = $_POST['quemAcompanha'];
...
if (isset($_SESSION['dados_pessoais']['com_quem_mora'])) {
    $_POST['comQuemMora'] = $_SESSION['dados_pessoais']['com_quem_mora'];
}
if (isset($_SESSION['dados_pessoais']['quem_acompanha'])) {
    $_POST['quemAcompanha'] = $_SESSION['dados_pessoais']['quem_acompanha'];
}
?>
<!DOCTYPE html>
<html lang="en">
...
//                                        echo $_SESSION['identificacao']['outros'];
//                                    } else {
//                                        echo "<font style='color: #cccccc;'>Outro</font>";
//                                    }
                                    ?>
                                    Responsável</font>
                                </div>
                                ...
                                <div class="controls" style='line-height: 20px;'>
                                    ...
                                    <label>
                                        <input type="radio" name="quemAcompanha" value="outro" id='outroRadio' required <?php
                                        if (!isset($_SESSION['identificacao']['nome_responsavel'])) {
                                            echo 'disabled';
                                        }
                                        ?>
                                        ...
                                        ?>><font style='position: relative; top: 3px; left: 0px; display: inline;' id='textoOutroR'>
                                               <?php
//                                               if (isset($_SESSION['identificacao']['outros'])) {
//                                                   echo $_SESSION['identificacao']['outros'];
//                                               } else {
...
//                                               }
                                ...
                                <?php } ?>
                                <a class='btn' href='identificacaoRematricula.php'>Voltar</a>
