  ...
  if( $inscricao == "" ){	  
	  foreach( $gdb->gs['CODIGO'] as $i => $value ){
		...
		if( $checkBox == 'on' ){
			...
			$gdb2->open(" UPDATE eventoInscricao SET tipo = 'M' WHERE codigoInscricao = '$value' ");
			$temEnvio = 1;
			...
			if ($gmailSender->smtpmailer($Destinatario, 'minhocanacabeca.comcap@gmail.com', utf8_decode($gdb->gs['NOME'][$i]), $titulo, $mensagem1)) {
						...
						<? 
							foreach($gdb->gs['CODIGO'] as $key=>$value){?>
								<tr>
								   ...
								   <td align="center" >
								   <? 
								   if( $gdb->gs['SITUACAO'][$key] == 'Enviado' ){ ?>
