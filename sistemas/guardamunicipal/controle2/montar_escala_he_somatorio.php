		...
		<legend class="negrito"><? echo $localT.' - '.$semanaT.' - '.$diaT." / ".$mesT." / ".$anoT.' - '.$horainicial.' as '.$horafinal; ?></legend>
			...
			if(isset($conf)) {
		   		foreach($conf as $loginF => $value){
		   			if($tamanho > 0){
						$query = "SELECT (select count(id) from listaescala where login='".$value."' and MONTH(data)=$mes_atual AND YEAR(data)=$ano_atual and idescala>0 and he=1) as total1, (select count(id) from listaescala where login='".$value."' and MONTH(data)=$mes_atual AND YEAR(data)=$ano_atual and idescala>0 and he=2) as total2";
