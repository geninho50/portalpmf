...
<?php
	...
	$queryA = "select * from horaextra where Length(guarda) > 0 and MONTH(data)='".$mes."' and YEAR(data)='".$ano."' order by DAY(data)";
...
<tr>
  <td width="23"  align="center" bgcolor="#E9E9E9"><a href="javascript:mudarAgendaInt(6,1,<?php echo $mesant; ?>,<?php echo $anoant; ?>);"><img src="images/seta_esq.jpg" width="23" height="25" border="0"></a></td>
  <td width="1198" align="center" bgcolor="#E9E9E9" class="style7"><?php echo "".mesesInt($mes)." | <FONT COLOR=\"#7A3635\">".$ano."</FONT>"; ?></td>
  <td width="24" align="center" bgcolor="#E9E9E9"><a href="javascript:mudarAgendaInt(6,1,<?php echo $mesprox; ?>,<?php echo $anoprox; ?>);"><img src="images/seta_dir.jpg" width="24" height="25" border="0"></a></td>
</tr>
<tr align="center">
  <td colspan="3">
	...
	for ($i=1,$d=1;$d<=$Dias;)
	{
		...
		for ($x=1;$x<=7 && $d <= $Dias;$x++,$i++)
		{
			if ($i > $Dia)
			{
				...
				$query = "select * from horaextra where Length(guarda) > 0 and MONTH(data)='".$mes."' and YEAR(data)='".$ano."' and DAY(data)='".$d."' order by DAY(data)";
	...
	<td>
			...
			<tr>
				<td width=25% align=left valign=top bgcolor="#006699" bordercolor="#FFFFFF" class="branco"><b>Matutino</b></td>
					<? 
					 $queryt = "SELECT * FROM horaextra where turno='MATUTINO' and data='$datac'";
			...
			<tr>
				<td width=25% align=left valign=top bgcolor="#006699" bordercolor="#FFFFFF" class="branco"><b>Vespertino</b></td>
					<? 
					 $queryt = "SELECT * FROM horaextra where turno='VESPERTINO' and data='$datac'";
					  $resultadot = $obj->executaQuery($queryt);
					  while($linhat = mysql_fetch_assoc($resultadot)){ 
							$idt = $linhat['id'];
							$nome = $linhat['guarda'];
...
<?php
		...
		{ 
			...
			$mesC = $datadinitemp[1];
			$anoC = $datadinitemp[0];
			$queryC = "select * from horaextra where Length(guarda) > 0 and MONTH(data)='".$mesC."' and YEAR(data)='".$anoC."' and DAY(data)='".$dC."' order by DAY(data)";
