   ...
   $dia_atual = substr($data_atual,8,2);
	...
	if(isset($conf)) {
  		foreach($conf as $loginF => $value){
			$query = "SELECT sum(hora1) as horat1,sum(hora2) as horat2 from listaescala where login='$value' and data BETWEEN '".$ano_atual."-01-01' AND '".$ano_atual."-".$mes_atual."-31' group by login";
