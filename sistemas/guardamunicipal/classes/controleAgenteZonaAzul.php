<?php
	...
	{
		// alterar
		$query = "UPDATE agente_zonaazul set nome='$palavranome',colocacao='$colocacao',dias='$dias',turno='$turno',datalimite='$datalimite' where id=$id";		
	}
	else
	if( $id > 0 )
	{
	...
	else
	{
		...
		if( $tamanho > 0 ){
			...
			if($dados)
			{
				...
				$query = "INSERT INTO agente_zonaazul (nome,colocacao,dias,turno,datalimite) values ('$palavranome','$colocacao','$dias','$turno','$datalimite')";
				$verIncluir = true;
			}
		}
	}
	...
	if( $verIncluir == false )
	{
		header ("Location:../controle/busca_agente_zonaazul.php");
	}
	else
	{
		header ("Location:../controle/cadastro_agente_zonaazul.php");
	}
?>