<?php
   ...
   if( $idGuarnicao == 0 )
   {
	...
	if( $chave == 1 )
	{
		...
		$queryJ4 = "insert into j8(idGuarnicao, hora_entrada, data_entrada,status) values ($idGuarnicao,'$hora_atual','$data_atual',0)";
		$conexao->executaQuery($queryJ4);
		
		$queryA = "insert into atividade (hora,data,dados)values('$hora_atual','$data_atual','$idGuarnicao - $vtrTemp, $guarda1 $guarda2 $guarda3 $guarda4 $guarda5 adicionada ao J8')";
		$conexao->executaQuery($queryA);
		
		echo "<script>alert('Guarnicao em J8!');</script>"; 
		echo "<script> window.location.href = '../controle/controle_guarnicao_disponivel.php' </script>";    
			...
			$queryA = "insert into atividade (hora,data,dados)values('$hora_atual','$data_atual','$idGuarnicao - $vtrTemp removida do J8')";
	 		$conexao->executaQuery($queryA);
			
			echo "<script>alert('Guarnicao removida do J8!');</script>"; 
