<?php
   ...
   if( $idGuarnicao == 0 )
   {
	...
	if( $chave == 1 )
	{
		...
		$queryA = "insert into atividade (hora,data,dados)values('$hora_atual','$data_atual','$idGuarnicao - $vtrTemp, $guarda1 $guarda2 $guarda3 $guarda4 $guarda5 adicionada ao J6')";
		$conexao->executaQuery($queryA);
		
		echo "<script>alert('Guarnicao em J6!');</script>"; 
		echo "<script> window.location.href = '../controle/controle_guarnicao_disponivel.php' </script>";    
			...
			$queryA = "insert into atividade (hora,data,dados)values('$hora_atual','$data_atual','$idGuarnicao - $vtrTemp removida do J6')";
			$conexao->executaQuery($queryA);
			
			echo "<script>alert('Guarnicao removida do J6!');</script>"; 
