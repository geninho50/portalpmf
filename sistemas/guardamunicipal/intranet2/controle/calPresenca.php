...
<?php
	...
	$queryA = "select * from chamada where MONTH(data)='".$mes."' and YEAR(data)='".$ano."' order by DAY(data)";
	...
	while ( $linhaA = mysql_fetch_array($resultadoA) )
	{
		$idagendaA = $linhaA['id'];
		$loginA = $linhaA['login'];
		$guardaA = $linhaA['guarda'];
		$chefeA = $linhaA['chefe'];
		$horaA = $linhaA['hora'];
		$data_cadastroA = $linhaA['data'];
		$dataA = $oData->formataDataPInterface($data_cadastro);
?>
<?php
	} // fim while	
?>
<tr>
  <td width="23"  align="left"><a href="javascript:mudarAgendaInt(11,1,<?php echo $mesant; ?>,<?php echo $anoant; ?>);"><img src="imagens/seta_esq.jpg" width="23" height="25" border="0"></a></td>
  <td width="461" align="center" bgcolor="#E9E9E9" class="style7"><?php echo "".mesesInt($mes)." | <FONT COLOR=\"#7A3635\">".$ano."</FONT>"; ?></td>
  <td width="24"><a href="javascript:mudarAgendaInt(11,1,<?php echo $mesprox; ?>,<?php echo $anoprox; ?>);"><img src="imagens/seta_dir.jpg" width="24" height="25" border="0"></a></td>
</tr>
<tr align="center">
  <td colspan="3">
	...
	for ($i=1,$d=1;$d<=$Dias;)
	{
		...
		for ($x=1;$x<=7 && $d <= $Dias;$x++,$i++)
		{
			if ($i > $Dia)
			{
				...
				$query = "select * from chamada where MONTH(data)='".$mes."' and YEAR(data)='".$ano."' and DAY(data)='".$d."' order by DAY(data)";
				...
				if ( $linha = mysql_fetch_array($resultado) )
				{
					...
					$hora = $linha['hora'];
					$datac = $linha['data'];
					$data = $oData->formataDataPInterface($data);
?>
<td style="background-color: rgb(135, 206, 235)"><a href="cadastro_chamada.php?&data=<? echo $datac; ?>"><FONT COLOR="white" size="2"><?php echo $d; ?></FONT></a></td>				
<?php
				}
				else
				{
		...
		{ 
			...
			$mesC = $datadinitemp[1];
			$anoC = $datadinitemp[0];
			$queryC = "select * from chamada where MONTH(data)='".$mesC."' and YEAR(data)='".$anoC."' and DAY(data)='".$dC."' order by DAY(data)";
			...
			if ( $linhaC = mysql_fetch_array($resultadoC) )
			{
				...
				$data = $oData->formataDataPInterface($data);
?>
<td style="background-color: rgb(135, 206, 235)"><a href="cadastro_chamada.php?data=<? echo $datac; ?>"><FONT COLOR="white" size="2"><?php echo $dini; ?></FONT></a></td><?php
			}
			else
			{
