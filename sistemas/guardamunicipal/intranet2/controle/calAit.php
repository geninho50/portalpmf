<?php
	ini_set('default_charset','UTF-8');
	

	class_exists('../classes/DB_mysql') || require_once("../classes/DB_mysql.php");
	class_exists('../classes/trataString') || require_once ("../classes/trataString.php");
	class_exists('../classes/trataArquivo') || require_once ("../classes/trataArquivo.php");
	class_exists('../classes/trataData') || require_once ("../classes/trataData.php");

	$obj = new DB_mysql;
	$objS = new trataString;
	$objT = new trataArquivo;
	$oData = new trataData;
	$conexao = $obj->conectarConf();

	$query = "";
	$normal = "style=color:#333333;";
	$hoje = "style=color:#FF0000;";
	$evento = "style=color:##990000;";
	$eventobgcolor = "bgcolor=\"#DFDFDF\"";
	$mes  = date("m");
	$dia  = date("d");
	$ano  = date("Y");
	$ano_ = substr($ano,-2);

	// Mes Par�mentro
	$mesp = $_GET['mes'];
	if( $mesp > 0 )
	{
		// Pegou...
	}
	else
	{
		$mesp = $_POST['mes'];
	}
	if( $mesp > 0 )
	{
		$mes = $mesp;
	}

	// Dia Par�mentro
	$diap = $_GET['dia'];
	if( $diap > 0 )
	{
		// Pegou...
	}
	else
	{
		$diap = $_POST['dia'];
	}
	if( $diap > 0 )
	{
		$dia = $diap;
	}

	// Ano Par�mentro
	$anop = $_GET['ano'];
	if( $anop > 0 )
	{
		// Pegou...
	}
	else
	{
		$anop = $_POST['ano'];
	}
	if( $anop > 0 )
	{
		$ano = $anop;
		$ano_ = substr($ano,-2);
	}

	function mesesInt($a) 
	{
		switch($a)
		{
			case 1:  $mescaseInt = "JANEIRO";   break;
			case 2:  $mescaseInt = "FEVEREIRO"; break;
			case 3:  $mescaseInt = "MAR�O";     break;
			case 4:  $mescaseInt = "ABRIL";     break;
			case 5:  $mescaseInt = "MAIO";      break;
			case 6:  $mescaseInt = "JUNHO";     break;
			case 7:  $mescaseInt = "JULHO";     break;
			case 8:  $mescaseInt = "AGOSTO";    break;
			case 9:  $mescaseInt = "SETEMBRO";  break;
			case 10: $mescaseInt = "OUTUBRO";   break;
			case 11: $mescaseInt = "NOVEMBRO";  break;
			case 12: $mescaseInt = "DEZEMBRO";  break;
		}
		return $mescaseInt;
	}

	// M�s Anterior
	$primeiro_dia_mes = 01;
	$dataant = $oData->somardiasData($ano."-".$mes."-".$primeiro_dia_mes,-1); // 2009-11-01
	$dataanttemp = explode("-", $dataant);
	$diaant = $dataanttemp[2];
	$mesant = $dataanttemp[1];
	$anoant = $dataanttemp[0];

	// Pr�ximo M�s
	$ultimodiames = $oData->retornaUltimoDiaMes($mes,$ano);
	$dataprox = $oData->somardiasData($ano."-".$mes."-".$ultimodiames,1); // 2009-11-01
	$dataproxtemp = explode("-", $dataprox);
	$diaprox = $dataproxtemp[2];
	$mesprox = $dataproxtemp[1];
	$anoprox = $dataproxtemp[0];
?>

<div id="mostraComboAgendaInt">
<?php include("incCarregandoInt.php");?>

<!-- ini mensagem imagem -->
<script type="text/javascript" src="js/wz_tooltip.js"></script>
<!-- fim mensagem imagem -->

<table width="504" border="0" cellspacing="0" cellpadding="0">
<tr>
  <td colspan="3">&nbsp;</td>
</tr>

<?php
	// Agenda deste M�s e Ano
	$queryA = "select * from receber_auto where MONTH(data_cadastro)='".$mes."' and YEAR(data_cadastro)='".$ano."' order by DAY(data_cadastro)";
	$resultadoA = $obj->executaQuery($queryA);
	//$nvaloresagendaint = 0;
	//$nvaloresagendaint = $obj->numregistros($queryA);
	while ( $linhaA = mysql_fetch_array($resultadoA) )
	{
		$idagendaA = $linhaA['id'];
		$loginA = $linhaA['login'];
		$matriculaA = $linhaA['matricula'];
		$numblocoinicialA = $linhaA['numblocoinicial'];
		$numblocofinalA = $linhaA['numblocofinal'];
		$letrainicialA = $linhaA['letrainicial'];
		$letrafinalA = $linhaA['letrafinal'];
		$quantidadeA = $linhaA['quantidade'];
		$horaA = $linhaA['hora'];
		$data_cadastroA = $linhaA['data_cadastro'];
		$dataA = $oData->formataDataPInterface($data_cadastroA);
?>
<?php
	} // fim while	
?>
<tr>
  <td width="23"  align="left"><a href="javascript:mudarAgendaInt(14,1,<?php echo $mesant; ?>,<?php echo $anoant; ?>);"><img src="imagens/seta_esq.jpg" width="23" height="25" border="0"></a></td>
  <td width="461" align="center" bgcolor="#E9E9E9" class="style7"><?php echo "".mesesInt($mes)." | <FONT COLOR=\"#7A3635\">".$ano."</FONT>"; ?></td>
  <td width="24"><a href="javascript:mudarAgendaInt(14,1,<?php echo $mesprox; ?>,<?php echo $anoprox; ?>);"><img src="imagens/seta_dir.jpg" width="24" height="25" border="0"></a></td>
</tr>
<tr align="center">
  <td colspan="3">
  
  <table width="504" border="1" cellpadding="1" cellspacing="1" bordercolor="#FFFFFF">

	  <tr align="center" bgcolor="#006699" class="style2">
		<td width="72" height="30">D</td>
		<td width="72">S</td>
		<td width="72">T</td>
		<td width="72">Q</td>
		<td width="72">Q</td>
		<td width="72">S</td>
		<td width="72">S</td>
	  </tr>


<?php
	$Data = strtotime($mes."/".$dia."/".$ano_);
	$Dia  = date('w',strtotime(date('n/\0\1\/Y',$Data)));
	$Dias = date('t',$Data);
	$dini = 0;
	$d = 0;
	for ($i=1,$d=1;$d<=$Dias;)
	{
		echo("<tr align=\"center\" bgcolor=\"#DFDFDF\" class=\"style2\">");
		for ($x=1;$x<=7 && $d <= $Dias;$x++,$i++)
		{
			if ($i > $Dia)
			{
				$destaque = '';
				$bgcolor = '';
				if ($d != $dia)    { $destaque = $normal; }
				if ($d == $dia) { $destaque = $hoje; }
				if (($x == 1) && ($d == $dia)) { $destaque = $hoje; }				
				
				$query = "select * from receber_auto where MONTH(data_cadastro)='".$mes."' and YEAR(data_cadastro)='".$ano."' and DAY(data_cadastro)='".$d."' order by DAY(data_cadastro)";
				$resultado = $obj->executaQuery($query);
				if ( $linha = mysql_fetch_array($resultado) )
				{
					$idagenda = $linha['id'];
					$login = $linha['login'];
					$matricula = $linha['matricula'];
					$numblocoinicial = $linha['numblocoinicial'];
					$numblocofinal = $linha['numblocofinal'];
					$letrainicial = $linha['letrainicial'];
					$letrafinal = $linha['letrafinal'];
					$quantidade = $linha['quantidade'];
					$hora = $linha['hora'];
					$datac = $linha['data_cadastro'];
					$data = $oData->formataDataPInterface($data_cadastro);
?>
<td style="background-color: rgb(135, 206, 235)"><a href="relatorio_qtd_dia.php?&data=<? echo $datac; ?>"><FONT COLOR="white" size="2"><?php echo $d; ?></FONT></a></td>				
<?php
				}
				else
				{
					echo("<td height=\"30\" ".$destaque.">".$d."</td>");
				}
				$d = $d + 1;
			}
			else { echo("<td> </td>"); }
		}
		for (;$x<=7;$x++)
		{ 
			$dini++;
			$datadini = $oData->somardiasData($oData->formataDataPBanco($data),$dini); // retona no formato: 2009-12-01
			$datadinitemp = explode("-", $datadini);
			$dC = $datadinitemp[2];
			$mesC = $datadinitemp[1];
			$anoC = $datadinitemp[0];
			$queryC = "select * from receber_auto where MONTH(data_cadastro)='".$mesC."' and YEAR(data_cadastro)='".$anoC."' and DAY(data_cadastro)='".$dC."' order by DAY(data_cadastro)";
			$resultadoC = $obj->executaQuery($queryC);
			if ( $linhaC = mysql_fetch_array($resultadoC) )
			{
				$idagendaC = $linhaC['id'];
				$login = $linhaC['login'];
				$matricula = $linhaC['matricula'];
				$numblocoinicial = $linhaC['numblocoinicial'];
				$numblocofinal = $linhaC['numblocofinal'];
				$letrainicial = $linhaC['letrainicial'];
				$letrafinal = $linhaC['letrafinal'];
				$quantidade = $linha['quantidade'];
				$hora = $linhaC['hora'];
				$datac = $linhaC['data_cadastro'];
				$data = $oData->formataDataPInterface($data_cadastro);
?>
<td style="background-color: rgb(135, 206, 235)"><a href="relatorio_qtd_dia.php?data=<? echo $datac; ?>"><FONT COLOR="white" size="2"><?php echo $dini; ?></FONT></a></td><?php
			}
			else
			{
				echo("<td height=\"30\"> ".$dini." </td>"); 
			}
		}
		echo("</tr>");
	}
?>
  </table></td>
</tr>
<tr>
  <td colspan="3">&nbsp;</td>
</tr>
</table>
</div>