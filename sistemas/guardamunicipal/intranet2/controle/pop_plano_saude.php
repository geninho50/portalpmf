...
<table width="100%"  border="1" cellpadding="1" cellspacing="1" bordercolor="#CCCCCC">
  <tr>
    <td width="318" align="right" bgcolor="#0086a8" class="branco">Lista de Plano de Saúde:</td>
    <td width="914" align="left">
	
	
	<table width="100%"  border="0" cellpadding="0" cellspacing="0">
    		<? 
			$chavet = true;
			$query = "SELECT * FROM planosaude where matricula=$matricula";
			$resultado = $conexao->executaQuery($query);
			while( $linha = mysql_fetch_array($resultado) )
			{
				$id = $linha["id"];
				$respostaplanosaude = $linha["planosaude"];
			?>
	  <tr bgColor="<?PHP if($chavet)
						...
						$chavet=!$chavet;
					?>">
        <td width="96%"><? echo'<font class=negrito>'.$respostaplanosaude.'</font><br>';?></td>
        </tr>
	  		<? 
			}
			?>
...
<table width="100%"  border="0">
  <tr>
    <td class="negrito" align="left">Informe abaixo caso haja mais algum plano de saúde. </td>
  </tr>
  <tr>
    <td class="negrito" align="left">
	<textarea name="xrespostaplanosaude" cols="100" rows="20" id="xrespostaplanosaude" onkeyup="converteUpper(this);"></textarea>	
	</td>
  </tr>
  <tr>
	<td align="left"  class="negrito">
			<input name="matricula" type="text" size="7" readonly="readonly" value="<? echo $matricula; ?>" />
			<input name="tipo" type="text" size="6" readonly="readonly" value="planosaude" /></td>
  </tr>
  <tr>
