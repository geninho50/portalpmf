<?php
   ...
   $hora_atual = date("H:i:s");
	...
	$queryC = "SELECT * FROM bloco WHERE idcaixa=$caixa order by data_baixa desc";
...
<table width="100%"  border="0" cellspacing="0" cellpadding="0">
  ...
  <tr>
	...
	<legend class="cabecalho">CONSULTA BLOCO POR CAIXA </legend>
		<form name="form1" method="post" action="relatorio_caixa.php" enctype="multipart/form-data"  onSubmit="return validaFormAll(this,'Pesquisar','Pesquisar')">
		
		<INPUT TYPE="hidden" name="cadastro" value="true">
		
		<table width="100%" border="0" cellspacing="1" cellpadding="1">
		    <tr>
		      <td width="10%" align="right" class="letra">Caixa:</td>
		      <td width="90%" align="left" class="letra">
              <select name="ycaixa" class="negrito">
                  <option value="0">Selecionar...</option>
                  <?php 
                        $queryU = "SELECT * FROM caixa order by caixa";
                        ...
                        while($linhaU = mysql_fetch_array($resultadoU))
                        {
							...
							$caixa = $linhaU['caixa'];
                      ?>
                  <option value="<?php echo $id; ?>"><?php echo $caixa; ?></option>
                  <?php 
                        } 
                      ?>
             </select>
