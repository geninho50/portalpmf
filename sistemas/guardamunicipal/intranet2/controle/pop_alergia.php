...
<table width="100%"  border="1" cellpadding="1" cellspacing="1" bordercolor="#CCCCCC">
  <tr>
    <td width="256" align="right" bgcolor="#0086a8" class="branco">Histórico de Alergia:</td>
	...
	<table width="100%"  border="0" cellpadding="0" cellspacing="0">
    		<? 
			$chavet = true;
			$query = "SELECT * FROM alergia where matricula=$matricula";
			$resultado = $conexao->executaQuery($query);
			while( $linha = mysql_fetch_array($resultado) )
			{
				$id = $linha["id"];
				$respostaalergia = $linha["alergia"];
			?>
	  <tr bgColor="<?PHP if($chavet)
						...
						$chavet=!$chavet;
					?>">
        <td width="96%"><? echo'<font class=negrito>'.$respostaalergia.'</font><br>';?></td>
        </tr>
	  		<? 
			}
			?>
...
<table width="100%"  border="0">
  ...
  <tr>
	...
	<textarea name="xrespostaalergia" cols="100" rows="20" id="xrespostaalergia" onkeyup="converteUpper(this);"></textarea>
	</td>
  </tr>
  <tr>
	<td align="left"  class="negrito">
			<input name="matricula" type="text" size="7" readonly="readonly" value="<? echo $matricula; ?>" />
			<input name="tipo" type="text" size="6" readonly="readonly" value="alergia" /></td>
  </tr>
  <tr>
