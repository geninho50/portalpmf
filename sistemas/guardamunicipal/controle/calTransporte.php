...
<?php
	...
	$queryA = "select * from transporte where Length(nome) > 0 and MONTH(data)='".$mes."' and YEAR(data)='".$ano."' order by DAY(data)";
...
<tr>
  <td width="23"  align="center" bgcolor="#E9E9E9"><a href="javascript:mudarAgendaInt(7,1,<?php echo $mesant; ?>,<?php echo $anoant; ?>);"><img src="images/seta_esq.jpg" width="23" height="25" border="0"></a></td>
  <td width="1198" align="center" bgcolor="#E9E9E9" class="style7"><?php echo "".mesesInt($mes)." | <FONT COLOR=\"#7A3635\">".$ano."</FONT>"; ?></td>
  <td width="24" align="center" bgcolor="#E9E9E9"><a href="javascript:mudarAgendaInt(7,1,<?php echo $mesprox; ?>,<?php echo $anoprox; ?>);"><img src="images/seta_dir.jpg" width="24" height="25" border="0"></a></td>
</tr>
<tr align="center">
  <td colspan="3">
	...
	for ($i=1,$d=1;$d<=$Dias;)
	{
		...
		for ($x=1;$x<=7 && $d <= $Dias;$x++,$i++)
		{
			if ($i > $Dia)
			{
				...
				$query = "select * from transporte where Length(nome) > 0 and MONTH(data)='".$mes."' and YEAR(data)='".$ano."' and DAY(data)='".$d."' and status=1 order by DAY(data)";
		...
		<td valign="top" style="background-color: rgb(135, 206, 235);"><FONT COLOR="white" size="2"><?php echo $d; ?></FONT>
			 ...
			 <table align=center valign=middle width=100%>
				<tr>
					<td>
					<? 
					  $queryt = "SELECT * FROM transporte where data='$datac' and status=1 order by hora asc";
					  ...
					  while($linhat = mysql_fetch_assoc($resultadot)){ 
							...
							$naoatentido = $linhat['motivonegado'];
							?>
							<tr>
								<td width=9% align=left class=negrito valign="top"><a href="cancelamento_transporte.php?idTransporte=<? echo $idt;?>&status=3"><? echo $hora;?>&nbsp;</A></td>
								<td width=91% align=left class=negrito>
								<a href="javascript:POPUP('vizualizar_transporte.php?idTransporte=<? echo $idt; ?>','800','400')"><font color="#006600"><? echo $nome; ?></font></a><br><hr>
								</td>
							</tr>
							<?
					}
...
<?php
		...
		{ 
			...
			$mesC = $datadinitemp[1];
			$anoC = $datadinitemp[0];
			$queryC = "select * from transporte where Length(nome) > 0 and MONTH(data)='".$mesC."' and YEAR(data)='".$anoC."' and DAY(data)='".$dC."' and status=1 order by DAY(data)";
