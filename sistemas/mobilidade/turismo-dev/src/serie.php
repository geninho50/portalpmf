...
$sql1 = "SELECT chave_acesso FROM turismo.viagens WHERE contratantes_email  = '$contratantes_email' LIMIT 1";
$resultado_sql1 = $conn->query($sql1);
$countsql1 = $resultado_sql1->rowCount();
if ($countsql1!=0){ 
foreach ($conn->query($sql1) as $row1) {
  $chave_acesso = $row1["chave_acesso"]; echo "tem cadastro";}
} else {$chave_acesso = substr(md5(mt_Rand()), 0, 6); echo "nao tem cadastro-nova chave";}
...
<body> <?php
        if ($count != 0) {
          echo "<META HTTP-EQUIV=REFRESH CONTENT = '0;URL=email_envia.php?cod_registro=" . $cod_registro . "&contratantes_email=" . $contratantes_email . "'>";
        } else {
            echo " 
