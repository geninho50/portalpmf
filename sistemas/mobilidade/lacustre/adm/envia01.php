...
$senha_ftp   = 'rm#1010';
$caminho = 'upload/lacustre/fichasdeviagem/';
...
$id_viagem = $id.".". $data_operacao .".". $horario_operacao .".". $sentido.".".$id_embarcacao;
...
if ( @ftp_put( $conexao_ftp, $destino, $arquivo_temp, FTP_BINARY ) ) {
	...
	$sql = "INSERT INTO lacustre.viagens
	(id, id_viagem, id_embarcacao, data_operacao, horario_operacao, data_criado, sentido, justificativa, modalidade,  imagem, status, contrato, num_ficha) VALUES
	(:id, :id_viagem, :id_embarcacao, :data_operacao,  :horario_operacao,  :data_criado, :sentido, :justificativa, :modalidade, :imagem, :status, :contrato, :num_ficha)";
	$stmt = $conn->prepare ($sql);
...
<html lang="pt-br">
	...
	<body> <?php
		if($count != 0){
			echo "
			<META HTTP-EQUIV=REFRESH CONTENT = '0;URL=form02.php?id_viagem=". $id_viagem . "&id=". $id. "' > 
					";	
		}else{
			echo "
