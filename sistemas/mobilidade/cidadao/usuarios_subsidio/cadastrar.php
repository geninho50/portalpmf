...
$sql = "SELECT id FROM sim.usuarios_subsidio WHERE id=(select max(id) from sim.usuarios_subsidio)";
...
elseif ($categoria == "Social Especial") {
    $id_sufix = 4;
    $data_validade = date('Y-m-d', strtotime('+1 year', strtotime($data_criado)));
    $data_validade_especial = $data_validade;
    $nis = ($_POST['nis']);
...
$id_cadastro = $id_sufix . str_pad($id, 6, '0', STR_PAD_LEFT);



//$observacoes = filter_input(INPUT_POST, 'observacoes', FILTER_SANITIZE_EMAIL); // sera anotado pelo validador SMPU em proximo passo

$query_03 = "SELECT * FROM sim.usuarios_subsidio where id_cadastro = '$id_cadastro'";
...
if ($count03 != 0) {
    ...
    $sql = 'INSERT INTO sim.usuarios_subsidio
		(
			nome_passageiro,
			cpf,
            rg,
    ...
    $query_02 = "SELECT * FROM sim.usuarios_subsidio where data_criado = '$data_criado'";
