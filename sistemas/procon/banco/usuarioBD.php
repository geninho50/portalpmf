...
$fileProcurador = $gdb->vargetpost('fileProcurador');
$fileDocumento	= $gdb->vargetpost('fileDocumento');
...
$gdb->open("select * from pessoa p , pessoaAuxiliar pa  where p.codigopessoa = pa.codigopessoa and cpf = '$cpf' and codigoprojeto = '$codigoprojeto' and pa.situacaoPROCON='U'  ");

if( $gdb->linhas > 0 ){
	$temCPF = 1;
}
	
$gdb->open("select * from pessoa p , pessoaAuxiliar pa  where p.codigopessoa = pa.codigopessoa and email = '$email' and codigoprojeto = '$codigoprojeto' and pa.situacaoPROCON='U'");
...
if( !$temCPF && !$temEmail ){
	...
	$extensao = "";
	$tipo = strtolower($_FILES['fileDocumento']['type']);
	...
	else if( strpos(strtolower($_FILES['fileDocumento']['type']),'jpeg' ) != false ) $extensao = 'jpeg';
	...
	if( basename($_FILES['fileProcurador']['name']) !=""  ){
		...
		else if( strpos(strtolower($_FILES['fileProcurador']['type']),'jpeg' ) != false ) $extensao2 = 'jpeg';
	}
	...
	*/
		 ...
		 $operacao = array( "success"=>"2","mensagem"=>"Problema com o arquivo, verifique se ele tem o tamanho igual ou menor que 400KB Ou se a extensão dele é JPG ou PDF ! tipo : $tipo");
