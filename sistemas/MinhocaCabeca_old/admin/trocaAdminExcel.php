...
$gdb->open("SELECT  m.id_pessoa, m.data_troca, p.nome, p.email, p.telefone, p.celular
FROM minhocario m
LEFT JOIN pessoa p ON p.id_pessoa = m.id_pessoa
ORDER BY m.id_pessoa, m.data_troca DESC");
...
$objPHPExcel->getProperties()->setCreator("Prefeitura Municipal de Florianopolis")
							 ...
							 ->setTitle("Lista de Trocas de Caixa Geral")
							 ->setSubject("Lista de Trocas de Caixa Geral")
							 ->setDescription("Lista de Trocas de Caixa Geral")
							 ->setKeywords("office 2007 openxml php")
                             ->setCategory("Lista de Trocas de Caixa Geral");
...
for ($i = 0; $i < count($data_troca); $i++){
	$objPHPExcel->setActiveSheetIndex(0)
				->setCellValue('A'.($i+2), $nome[$i])
				->setCellValue('B'.($i+2), $email[$i])
				->setCellValue('C'.($i+2), $telefone[$i])
				->setCellValue('D'.($i+2), $celular[$i])
				->setCellValue('E'.($i+2), date('d/m/Y', strtotime($data_troca[$i])));
...
// Rename worksheet
$objPHPExcel->getActiveSheet()->setTitle('Trocas de Caixa Geral');
