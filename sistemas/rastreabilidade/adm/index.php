...
<body>
<form name="login" action="agenda.php">
...
<link rel="stylesheet" href="../../../layout/themePMF/css/style.css">
	    ...
	    <ul class="mini-header__items">
	      <li style="font-size: 13px;">ADM - Capacitação Rastreabilidade</li>
	    </ul>
	  </div>
	  ...
	  <li class="mini-header__social-mobile">
		...
		<div class="flex-container hero-wrapper" id="solicite" >
			<div id="busca-home" class="search-bar search-bar--home column6-lg column8-sm column8-xs">
		        <h1 class="hidden-sm hidden-xs">Entrar no ambiente de administração</h1><br>
		        <p style="color: white;">Inclua seu cpf e senha.</p>
		 		...
		 		<div class="row">
		            ...
		            <div class="col-md-3">
		                <label>Senha:</label><input value="" id="senha" name="senha" class="form-control" type="password" />
		            </div>                  
				</div><br>		
  ...
  $("#cpf").mask("999.999.999-99");
     ...
     function enviar(){
		if( $('#cpf').val() == "" ){
			...
			$.ajax({			
				   type: "POST",
				   url: "../banco/logarADM.php",
				   dataType: "json",
				   data: obj,
				   success: function ( data ) {
				   	console.log(data);
					   if( data['success'] == 1 ){
						   alert("CPF encontrado, entrando!");
               $('#senha').val("");
