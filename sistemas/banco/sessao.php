   ...
   function iniciar_sessao($sistema = '', $usid){
	 ...
	 if( ( $db->gs['LOGADO'][0] == 0 ) || ( !isset($_SESSION) ) ){
		...
		if( $db->linhas>0 ){	
			...
			setcookie("CODIGOPROJETO", '$sistema', 0, '/');
			setcookie("EMAIL", $db->gs['LOGIN'][0], 0, '/');
			setcookie("CPF", $db->gs['CPF'][0], 0, '/');         
			setcookie("SESSAO", $codigoSessao, 0, '/');
		    $db->open("insert into sessao( codigoSessao,   dataInicial, horainicial, codigoUsuario, numeroip, sistema ) 
   			   					   values('$codigoSessao','$data',     '$hora',     '$usid',        '$nu_ip','$sistema') ");
		}									   
	 }else return 0;
