...
class gdb {
     ...
     function conexao( $banco ='backend',
     ...
     function open( $ds_sql='',
        ...
        if( !$this->conexao_lg ){
           ...
           $this->conexao('backend','192.168.1.20','root','https!@17');
        }
     ...
     function print_tabela( $titulo = "", 
         ...
         //  Montagem do vetor de alinhamento
          ...
          <div class="container">  
          <div class="row">
          <div class="col-lg-<?php print (2 * $cl_visb["V"]); ?>">
          <div class="well">
		  <table class="table table-striped table-bordered table-hover "
                 width="<?php print $this->largura; ?>" border=1 cellpadding="<?php print $cl_visb["V"]; ?>" cellspacing='0' >
          <tr align="center"  >
          <td colspan="<?php print $cl_visb["V"]; ?>">                                                       
               <a href="#" class="list-group-item active">
                <b><?php	if( !$in_incl ) print utf8_encode($titulo);	else print $titulo;?></b>
              </a>
            </td>
          </tr>
          <tr>
          ...
          ?> <th class=titulo onMouseOver="this.style.cursor='pointer';" 
                              OnClick="OrdemColuna(<?php print $xcol; ?>);"  ><?php if( !$in_incl ) print utf8_encode($xcoluna); 
		 ...
		 // Setor de configura??o da fun??o escolher.	  
          for( $xlin=0 ;$xlin<$this->linhas ; $xlin++ ){
               ...
               <tr 
                   OnClick="escolher(<?php print $dado; ?>);" 
               ...
               <?php
                for( $xcol=0 ;
                     ...
                     if ( $this->vt_visv[$xcol]=="V" ) {
                       ?><td align=<?php if ($this->vt_alnh[$xcol]=="C")     print "center";
       ...
       <?php } ?>
          <tr><td  colspan="<?php print $cl_visb["V"]; ?>"><b><?php print "Total de Registro : ".$this->linhas; ?></b></td></tr>
		  <?php if( $nomeBotaoConsulta !="" ){ ?>
			  <tr align="left"  >
				<td colspan="<?php print $cl_visb["V"]; ?>">                                                       
				 <input type=button class="btn btn-default btn-small" value="<?php print $nomeBotaoConsulta; ?>" onclick="consultar(0)">
				 <?php if( $tem_botao_novo == 1  ){ ?>
  ...
  function buscarCodigoPessoa($cnpjCpf, $codigoProjeto ){
     ...
     $this->open("SELECT p.codigopessoa as codigo 
                    FROM pessoa p, 
                        pessoaAuxiliar pa 
				  WHERE pa.codigopessoa = p.codigopessoa 
				    AND pa.situacaoPROCON = 'F' 
				    AND pa.codigoProjeto = '$codigoProjeto' 
				    AND $documento =  $cnpjCpf ");
