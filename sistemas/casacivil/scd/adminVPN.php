	...
	<head>
  	...
  	<body>
		...
		<div class="overflow" id="finalizado">
  		...
  		<div class="overflowResultProc">
	  		<table class="table table-striped table-bordered text-capitalize" id="tabelaProcurar">
	  		</table>
  		</div>
		...
		$('.porcurarBot').bind('click',function(){
			...
			var obj = { campo : campo };
			$.post( "backend/SELprocuraVPN.php", obj).done(function( data ) {
				var retorno = jQuery.parseJSON(data);
				if(retorno.success == 1){
					$('#tabelaProcurar').html(retorno.resultado);
				}
			});
		});
