...
function dadosveiculo($placa)
{
	...
	if($resultado != FALSE)
	{
		...
		else
		{
			...
			$permissionario_id=pg_result($resultado,0,"permissionario_id");
	...
	//		echo "<td align='left'><a class='smt'>".utf8_decode($permissionario)."</a></td>";
			...
			echo "</tr>";
			listacondutores($permissionario_id);
			if($obs=="" || $obs==null)
			{
			}
			else
			{
				echo "<td align='center' width='100'><font face='verdana' size='1'>Observa&ccedil&atildeo</td>";
				echo "<td align='left' width='300'><a class='smt'>".utf8_encode($obs)."</a></td>";
			}
			echo "</table>";
		}
	}
...
function id_perm($placa)
{
	//echo "IDVEICPLACA";
	$sql = "SELECT permissionario_id";
	$sql = $sql." FROM vencimentos WHERE placa like '%$placa%'";
	$resultado=pg_query($sql);
	//$varid=-1;
	if($resultado != FALSE)
	{
		...
		if($linhas==1)
		{
			$varid=pg_result($resultado,0,"permissionario_id");
		}
		elseif($linhas==0)
		{
			$varid=-1;
	...
	listacondutores($varid);
	//return $varid;
}

function listacondutores($idperm)
{
	//echo "Perm";
	//echo $idperm;
	$sql = "SELECT condutor";
	$sql = $sql." FROM perm_condutores WHERE perm = '$idperm' and dataini <= current_date and datafim is null ";
	$resultado=pg_query($sql);
	if($resultado != FALSE)
	{
		$linhas=pg_num_rows($resultado);
		...
		{	//echo "Linhas";
			...
			for ($i=0; $i<$linhas; $i++)
			{
				$idcond=pg_result($resultado,$i,"condutor");
				//echo "id";
				//echo $idcond;
				$sql = "SELECT cpf, condutornome FROM condutores WHERE id = '$idcond'";
				$condresultado=pg_query($sql);
				if($condresultado != FALSE)
				{
					$condlinhas=pg_num_rows($condresultado);
					if($condlinhas==1)
					{
						$condcpf=pg_result($condresultado,0,"cpf");
						$condnome=pg_result($condresultado,0,"condutornome");
						if($condcpf != "")
						{
							$condcpf=substr($condcpf,0,3).'.'.substr($condcpf,3,3).'.'.substr($condcpf,6,3).'-'.substr($condcpf,9,2);
						}
						else
						{
							$condcpf="";
						...
						echo "<tr>";
						echo "<td align='center' width='100'><font face='verdana' size='1'>Condutor CPF</td>";
						echo "<td align='left' width='300'><a class='smt'>$condcpf </a></td>";
						echo "</tr><tr>";
						echo "<td align='center' width='100'><font face='verdana' size='1'>Condutor Nome</td>";
						echo "<td align='left' width='300'><a class='smt'>".utf8_encode($condnome)."</a></td>";
						echo "</tr>";
					}
				}
			}
