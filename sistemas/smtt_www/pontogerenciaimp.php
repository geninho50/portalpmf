...
$sql = "SELECT nome, funcao, id_ponto, ponto.usuario_id, dataref, horaref, date_part('day',dataref) as dia, date_part('month',dataref) as mes,";
$sql = $sql." date_part('year',dataref) as ano, ipref, entsai FROM ponto INNER JOIN usuarios";
$sql = $sql." ON ponto.usuario_id = usuarios.usuario_id WHERE usuarios.nome $filtrofiscalnome";
...
$sql = $sql." ON ponto_formatado.usuario = usuarios.usuario_id WHERE usuarios.nome $filtrofiscalnome";
...
if($resultado != FALSE)
{
	...
	if($fiscalnome != "")
	{
		...
		echo " <p align='center'> <font face='verdana' size='5'>$funcao</p></font>";
...
//		$hsf=strval(intval($hs)).':'.$mn;
	...
	echo "<table align='center' width='450' border=0 bordercolor='#9ACD32'>";
	echo "<tr>";
	echo "<td width='150' align='left'><a class='tab'>Média (hs/dia):</td>";
