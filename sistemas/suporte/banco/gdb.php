    ...
    if( strstr( strtoupper( $ds_sql ), strtoupper( "insert" ) ) !='' || 
     ...
     function print_tabela( $titulo = "", 
         ...
         //  Montagem do vetor de alinhamento
          ...
          <div class="container">  
          <div class="row">
          <div class="col-lg-<?= (2 * $cl_visb["V"]); ?>">
          <div class="well">
      <table class="table table-striped table-bordered table-hover "
                 width="<?= $this->largura; ?>" border=1 cellpadding="<?= $cl_visb["V"]; ?>" cellspacing='0' >
          <tr align="center"  >
          <td colspan="<?= $cl_visb["V"]; ?>">                                                       
          ...
          ?> <th class=titulo onMouseOver="this.style.cursor='pointer';" 
                              OnClick="OrdemColuna(<?= $xcol; ?>);"  ><?php if( !$in_incl ) print utf8_encode($xcoluna); 
     ...
     // Setor de configura??o da fun??o escolher.   
       ...
       <?php } ?>
          <tr><td  colspan="<?= $cl_visb["V"]; ?>"><b><?= "Total de Registro : ".$this->linhas; ?></b></td></tr>
      <?php if( $nomeBotaoConsulta !="" ){ ?>
        <tr align="left"  >
        <td colspan="<?= $cl_visb["V"]; ?>">                                                       
         <input type=button class="btn btn-default btn-small" value="<?= $nomeBotaoConsulta; ?>" onclick="consultar(0)">
