...
<head>
  ...
  <?php
//header("location:http://www.pmf.sc.gov.br/sistemas/temporada/");
  //die();
	...
	$gdb->open("select c.dia, 
		               ...
		               c.idcursos as chave 
				        FROM cursos c
				        WHERE c.vagas > ( select count(*) from inscritos g Where g.idcursos = c.idcursos ) order by c.dia");
...
<body>
<form name="formCadastro" METHOD="GET" action="sucesso.php">
	...
	if ($iphone || $ipad || $android || $palmpre || $ipod || $berry || $symbian == true){
		...
		echo '<div class="flex-container hero-wrapper" style="background-image: url(../rastreabilidade/imagens/praia.jpg);">';  };    ?>
				...
				<div id="busca-home" class="search-bar search-bar--home column6-lg column8-sm column8-xs">
			        <h1 class="hidden-sm hidden-xs">Treinamentos Ambulantes - Operação Verão 2019-2020</h1><br>
		...
		<div class="flex-container hero-wrapper" id="solicite" >
			<div id="busca-home" class="search-bar search-bar--home column6-lg column8-sm column8-xs">
		        <h1 class="hidden-sm hidden-xs">Oi<span style="color: #FDAE45;"> <?php echo $nome ?> </span>escolha o dia, horário e local do seu treinamento:</h1><br>


    <h2>Escolha o dia e local</h2>
		<div class="row">
			<div class="col-md-6">
			    <select class="form-control" id="idcursos">
			      <?php foreach( $gdb->gs['CHAVE'] as $key=>$value ){ 	
            $idcursos = $value;
						print "<option value='$value'>".$gdb->gs['DIA'][$key]." - ".$gdb->gs['HORARIO'][$key]." - ".$gdb->gs['LOCAL'][$key]."</option>";
						}
				   ?>
			    </select>
			</div>
      <p style="color: white;">Caso o dia de sua preferência não esteja nas opções ao lado, significa que as vagas já se esgotaram. </p>
...
<br>
			<div class="row">
			 <div class="col-md-12">
				<input type="button" class="btn btn-primary botao" value="Entrar" onclick="salvar();" />
			</div>
			</div>
		</div>
</div>  
