...
$where = ($ativoAnimal<2?'where a.ativo = '.$ativoAnimal:'LEFT JOIN adoteDibea.interesse i ON i.id_animal = a.id_animal where a.ativo = "1" AND i.status="1"');

if (!empty($idAnimalBusca)) {
    $ativoAnimal = $_POST['ativoAnimal'];
    $where .= " AND a.id_animal = ".$idAnimalBusca;
} else {
    $where .= "";
}
...
LEFT JOIN adoteDibea.galeria g ON g.id_galeria = ga.id_galeria
$where
LIMIT $inicio, $limite");  //a.ativo = '$ativoAnimal'
...
<body>
    ...
    <form id="frm_animal" method="POST" action="edicao.php">
        <input type="hidden" id="id_animal_a" name="idAnimal">
        <input type="hidden" id="ativo" name="ativoAnimal">
        <input type="hidden" name="user" value="<?=$user;?>">
        <input type="hidden" name="password" value="<?=$password;?>">
    </form>
    ...
    <div class="container text-center">
        ...
        <a  href="#" onclick="changeExibicao(2)">
            <div style="width: 100px; height:100px; margin-left: 30px !important;font-size: 25px !important;border: 1px solid !important;border-radius: 86px !important;display: inline-block !important;" class="center" id="shiva2">
    ...
    <script>
    $( function() {
        var id = document.getElementById('id_animal_a');
        var ativo = document.getElementById('ativo');
        $( "#autocomplete" ).autocomplete({
        source: '../banco/getNomesEdicao.php',
        minLength: 1,
        select: function( event, ui ) {
            document.getElementById('id_animal_a').value = ui.item.id; document.getElementById('ativo').value = ui.item.ativo; document.getElementById('frm_animal').submit()
        }
        });
    });
    </script>
