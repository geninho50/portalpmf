...
$id_pessoa = $gdb->enviarArquivo($nm_arq_doc_rg,$nm_arq_doc_cpf,$nm_arq_doc_residencia);
if($id_pessoa != 0){
	...
	if(!move_uploaded_file($doc_rg,$targetPathDocRg)){
		echo json_encode(array('error' => 'Ocorreu um erro ao enviar o arquivo RG.'));
		die;
	}

	if($doc_cpf !='0'){
		if(!move_uploaded_file($doc_cpf,$targetPathDocCpf)){
			echo json_encode(array('error' => 'Arquivo enviado com sucesso, no entanto ocorreu um erro ao realizar o upload do Arquivo RG, por favor edite o pedido para substituir o arquivo.'));
			die;
		}
	}
	...
	if($doc_residencia !='0'){
		if(!move_uploaded_file($doc_residencia,$targetPathDocResidencia)){
			echo json_encode(array('error' => 'Arquivo enviado com sucesso, no entanto ocorreu um erro ao realizar o upload do Arquivo Comprovante de Residência, por favor edite o pedido para substituir o arquivo.'));
			die;
		}
	}
	...
	echo json_encode(array('error' => 'Erro ao enviar arquivo!'));
}
