...
$query		= "SELECT intranet_perfil_id FROM intranet_perfil WHERE intranet_perfil_id = $idperfil";
$result		= $drive->pedido($query);
$rows		= pg_num_rows($result);
if($rows == 1){
	$query			= "UPDATE intranet_perfil SET intranet_perfil_nome = '$txtPerfil' WHERE intranet_perfil_id = $idperfil";
	$result 		= $drive->pedido($query);
	$ultimoregistro = (int)$idperfil;
