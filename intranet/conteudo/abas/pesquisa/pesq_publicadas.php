...
require_once("../scripts/php/paginacao.php");
	...
	$numSql   = "SELECT COUNT(*) FROM intranet_pagina WHERE intranet_pagina_entidade_id = $TentId AND intranet_pagina_status = 't'";
    $sqlPub   = "SELECT * FROM intranet_pagina WHERE intranet_pagina_entidade_id = $TentId AND intranet_pagina_status = 't' ORDER BY intranet_pagina_id DESC LIMIT 10 OFFSET $inicio";				
    $Tcaminho = "?pagina=pagcad&menu=".$_GET['menu']."&aba=publicadas";
	
	$Treturn  = $drive->pedido($sqlPub); 
	$TreturnSqlNum = $drive->pedido($numSql); 
    
    echo "<div id=\"conteudo_publicadas\">";
    	$i = 0;
        while($Tpub = pg_fetch_object($Treturn)){				
            echo "
            <form method=\"post\">
            <input type=\"hidden\" name=\"idPagina\" value=\"".$Tpub->intranet_pagina_id."\" />
            <div class=\"container_item_result\">
                <span class=\"titulo_linkserv\">".$Tpub->intranet_pagina_titulo."</span>
                <br />
                    ".substr($Tpub->intranet_pagina_texto, 0, 150)." ...                           
                <br />
                <span align=\"absmiddle\"><a href=\"inicio.php?pagina=pagedit&menu=".$_GET['menu']."&idPag=".$Tpub->intranet_pagina_id."\"><img src=\"../layout/imagens/atualiza_btn_editar.png\" alt=\"editar\" width=\"50\" height=\"18\" border=\"0\" /></a></span>
                ...
                <span align=\"absmiddle\"><input type=\"image\" src=\"../layout/imagens/atualiza_btn_despublicar.jpg\" name=\"btPub\" id=\"btPub\" value=\"btPub\"  />	</span>
            </div>
            </form>";				
        $i++;
		} 
...
//------------------------------
	...
	if ($Treturn == true){		
		echo"<script>alert(\"Pagina despublicada com Sucesso!\");</script>";
		echo("<script>window.location = \"inicio.php?pagina=pagcad&menu=".$_GET['menu']."&aba=publicadas\";</script>");
	}else{		
		echo"<script>alert(\"Não foi possivele despublicar a Pagina.\");</script>";
		echo("<script>window.location = \"inicio.php?pagina=pagcad&menu=".$_GET['menu']."&aba=publicadas\";</script>");
	}	
}
